...
#include <TargetConditionals.h> /*G.Barrand*/
...
/*G.Barrand : #if defined(WITH_NEXT_FRAMEWORK) || (defined(__APPLE__) && defined(Py_ENABLE_SHARED)) */
...
#if TARGET_OS_IPHONE
extern char **environ; /* To release on iOS AppStore, we can't use _NSGetEnviron() which is a private API.*/
#else
/* On Darwin/MacOSX a shared library or framework has no access to environ directly,
   we must obtain it with _NSGetEnviron(). See also man environ(7). */
#include <crt_externs.h> /*G.Barrand : for NSGetEnviron().*/
#endif

/*G.Barrand : static char **environ;*/
...
#if TARGET_OS_IPHONE
#else
    char** environ = *_NSGetEnviron(); /*G.Barrand*/
#endif
#endif
    if (environ == NULL)
        return d;
