/*
 * $Id: kkern.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kkern.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.6  2010/10/19 08:40:02  barrand
 * *** empty log message ***
 *
 * Revision 1.5  2007/03/14 09:17:40  barrand
 * *** empty log message ***
 *
 * Revision 1.4  1999/12/03 15:42:30  barrand
 * *** empty log message ***
 *
 * Revision 1.3  1999/11/30  13:42:43  barrand
 * *** empty log message ***
 *
 * Revision 1.2  1999/11/08  11:33:09  barrand
 * *** empty log message ***
 *
               ...
               size_t len )
{
  ...
  if( len >= (sizeof blanks) * 4 ) {
    ...
    /*GB switch( (int)&str[len] & ( (sizeof blanks) - 1 ) ) { */
    switch( (long)&str[len] & ( (sizeof blanks) - 1 ) ) { /*GB*/
    case 7:
  ...
  if( cstr != NULL ) {
    ...
    if( cstr[i] != '\0' ) {
      while( cstr[i] == ' ' )
        i++;
      if( cstr[i] != '\0' ) {
        kprintf(" *** A character parameter has been defined with insufficient length = %d\n",
               len );
      }
    }
#endif
...
double fstrtod( const char *str,
               char **tail )
{
  double value = kstrtod( str, tail ); /*G.Barrand*/
...
/*GB #ifndef HAVE_MEMMOVE*/
/*
 * memory move with possible overlap
 */
void *kmemmove( void *vdst,          /*GB*/
              const void *vsrc,
              size_t n )
{
  void* tmp;
 ...
 */
int ku_strcasecmp( const char *str1,
               const char *str2 )
{
  return ku_strncasecmp( str1, str2, strlen( str2 ) + 1 );
}
 ...
 */
int ku_strncasecmp( const char *str1,
                const char *str2,
                size_t n )
{
  while( n > 0 ) {
 ...
 */
char *kuip_strdup( const char *str )
{
...
char *str2dup( const char *str1,
              const char *str2 )
{
  return mstrcat( kuip_strdup( str1 ), str2 );
}
...
char *str3dup( const char *str1,
              ...
              const char *str3 )
{
  return mstr2cat( kuip_strdup( str1 ), str2, str3 );
}
...
char *str4dup( const char *str1,
              ...
              const char *str4 )
{
  return mstr3cat( kuip_strdup( str1 ), str2, str3, str4 );
}
...
char *str5dup( const char *str1,
              ...
              const char *str5 )
{
  return mstr4cat( kuip_strdup( str1 ), str2, str3, str4, str5 );
}
