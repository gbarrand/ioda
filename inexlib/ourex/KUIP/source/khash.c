/*
 * $Id: khash.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: khash.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.3  2003/11/04 09:54:38  barrand
 * *** empty log message ***
 *
 * Revision 1.2  1999/11/08 11:25:39  barrand
 * *** empty log message ***
 *
                 ...
                 const void* p2 )
{
  HashArray *ent1 = (HashArray*)p1;
  HashArray *ent2 = (HashArray*)p2;
  return ku_strcasecmp( ent1->name, ent2->name );
}
 ...
 */
HashTable *khash_create( int size )
{
  HashTable *table = (HashTable*)malloc( sizeof(HashTable) );
  table->size = size;
  table->entries = (HashEntry**)calloc( size, sizeof(HashEntry*) );
  table->nentries = 0;
  khash_config( table, "string" );
  khash_config( table, "keep" );
  khash_config( table, "ignore" );
  return table;
}


void khash_config( HashTable* table,
                 const char* option )
{
  if( strcmp( option, "string" ) == 0 ) {
    ...
    table->strcmp = ku_strcasecmp;
    table->hash_cmp = hash_casecmp;
  }
  else {
    kprintf( " Invalid option %s for khash_config\n", option );
  }
}
 ...
 */
void khash_destroy( HashTable *table )
{
  khash_clear( table );
 ...
 */
void khash_insert( HashTable *table,
                 const char *name,
                 const void *value,
                 int tag )
{
 ...
 */
void khash_clear( HashTable *table )
{
  int i;
 ...
 */
void *khash_remove( HashTable *table,
                  const char *name )
{
  int h = hashpjw( table, name );
  HashEntry *ent;
 ...
 */
void *khash_lookup( HashTable *table,
                  const char *name,
                  int *tag_return )
{
  int h = hashpjw( table, name );
 ...
 */
int khash_entries( HashTable *table )
{
 ...
 */
HashArray *khash_array( HashTable *table )
{
  ...
  if( table->nentries > 0 ) {
    ...
    qsort( linear, n, sizeof(HashArray), 
