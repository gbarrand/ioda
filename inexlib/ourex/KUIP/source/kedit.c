/*
 * $Id: kedit.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kedit.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.4  2010/03/24 10:35:52  barrand
 * *** empty log message ***
 *
 * Revision 1.3  2009/06/23 08:01:23  barrand
 * *** empty log message ***
 *
 * Revision 1.2  1999/11/08 11:24:22  barrand
 * *** empty log message ***
 *
            ...
            int use_server )
{
  ...
  if( use_server && kc_flags.use_server ) {
...
#ifdef APOLLO
    if( ku_strcasecmp( kc_value.set_host_editor, "DM" ) == 0 ) {
    ...
    if( ku_strncasecmp( kc_value.set_host_editor, "EDT", 3 ) == 0
       || ku_strncasecmp( kc_value.set_host_editor, "TPU", 3 ) == 0 ) {
      ...
      if( ku_strcasecmp( kc_value.set_host_editor, "EDT" ) == 0 ) {
...
# ifdef UNIX
    ...
    if( status == 130
      ...
      kprintf( " *** Bogus non-zero exit code ignored\n" );
      status = 0;
    }
# endif /* UNIX */
...
#ifdef APOLLO
  if( ku_strcasecmp( kc_value.set_host_pager, "DM" ) == 0 ) {
...
int kxhosteditor()
{
  ...
  if( strcmp( editor, "?" ) == 0 ) {
    kprintf( " Current host editor is '%s'\n", kc_value.set_host_editor );
  }
  else if( ku_strcasecmp( editor, "WINDOW" ) == 0 ) {
  ...
  else if( ku_strcasecmp( editor, "PAD" ) == 0 ) {
...
int kxhostpsviewer()
{
  ...
  if( strcmp( psviewer, "?" ) == 0 ) {
    kprintf( " Current PostScript Viewer  is '%s'\n",
...
int kxhostpager()
{
  ...
  if( strcmp( pager, "?" ) == 0 ) {
    kprintf( " Current host pager is '%s'\n", kc_value.set_host_pager );
  }
  else {
...
int kxhostprinter()
{
  ...
  if( strcmp( cmd, "?" ) == 0 ) {
    if( kc_value.set_print_cmds == NULL ) {
      kprintf( " No host printer command defined.\n" );
    }
    else {
      int i;
      kprintf( " Filetype | Host printer command\n" );
      kprintf( " ---------|---------------------\n" );
    ...
    for( i = 0; kc_value.set_print_cmds[i] != NULL; i++ ) {
      if( ku_strcasecmp( kc_value.set_print_exts[i], ext ) == 0 ) {
        free( kc_value.set_print_cmds[i] );
...
int kxprint()
{
  if( kc_value.set_print_cmds != NULL ) {
    ...
    if( stream == NULL ) {
      kprintf( " *** KUIP/PRINT: file %s not found\n", file );
      return -1;
    }
    else
      fclose( stream );
    ...
    for( i = 0; kc_value.set_print_cmds[i] != NULL; i++ ) {
      ...
      if( ku_strncasecmp( file_end - n, kc_value.set_print_exts[i], n ) == 0 ) {
  ...
  kprintf( " *** KUIP/PRINT: no host printer command defined\n" );
  return -1;
}
...
int kxcolumns()
{
  ...
  if( width == 0 )
    kprintf( " Current number of columns: %d\n", kc_window.term_width );
  else {
    if( width < 40 ) {
      kprintf( " *** COLUMNS: minimum screen width is 40\n" );
      width = 80;
    }
    sprintf( kc_value.set_columns, "%d", width );
    kc_window.term_width = width;
#ifndef IBM370
    /*GB kgl_setwidth( width );*/
    if( kjmpaddr.kgl_setwidth!=NULL) (*kjmpaddr.kgl_setwidth)(width);
...
# else
    ...
    kprintf( " *** PSVIEW not implemented\n" );
    status = -1;
...
# endif
    ...
    kprintf( " *** PSVIEW: file %s not found\n", file );
    status = -1;
  }
  return status;
}
