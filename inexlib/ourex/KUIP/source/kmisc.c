/*
 * $Id: kmisc.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kmisc.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.2  1999/11/08 11:31:00  barrand
 * *** empty log message ***
 *
              ...
              const char *ftype ) /* default file extension */
{
  ...
  if( path[0] == '$' || path[0] == '~' ) {
    ...
    if( path[0] == '$'
      ...
      kprintf( " *** Environment variable %s not defined\n", path );
    }
# ifndef MSDOS
    else if( path[0] == '~' ) {
      struct passwd *pwd = getpwnam( &path[1] );
      if( pwd == NULL ) {
        kprintf( " *** User %s not found in password file\n", &path[1] );
      }
      else {
        repl = pwd->pw_dir;
      }
...
int ku_system(const char* a_cmd) /*G.Barrand.*/
{
#if TARGET_OS_IPHONE
  kprintf(" *** system() not available on iOS. Cant't execute \"%s\" ***\n",a_cmd);
  return EXIT_FAILURE;
#else
  return system(a_cmd);
#endif  
...
/* if execl returns , this is an error */
    kprintf( " *** Cannot exec host_shell (%s) ***\n", host_shell );
    exit(1);
  }
  else if( pid > 0 ) {
...
# endif
  ...
  if( shell_cmd[0] == '\0' ) {
    if( kc_flags.in_motif ) {
      kprintf( " Cannot run shell from Motif mode\n" );
      return -1;
    }
    else {

#ifdef IBMVM
      kprintf( " Type 'return' to return to the command processor\n" );
      ku_system( "SUBSET" );
#define _done_
#endif
...
#define _done_
      ...
      if( strcmp( kc_value.set_host_shell, "/com/sh" ) == 0 ) {
        ...
        kprintf( " Type 'exit' to return to the command processor\n" );
        if( ku_system( kc_value.set_host_shell ) == -1 )
...
# endif
    ...
    status = ku_system( cmd_string );
    if( status >= 0 && status <= 4 )
      status = 0;               /* ignore informational messages */
    else
      kprintf( " Command '%s' returned status code %d\n", cmd_string, status );
    free( cmd_string );
#define _done_
#endif
...
#define _done_
    ...
    if( strcmp( kc_value.set_host_shell, "/com/sh" ) == 0 ) {
      ...
      if( status == -1 )
        ...
        kprintf( " *** %s returned exit code %d\n", shell_cmd, status );
    }
#define _done_
#endif
...
# endif
        kprintf( " *** %s returned status code %d\n", shell_cmd, status );
    }
#endif
  }
  return status;
...
int kxlcdir()
{
  ...
  ku_alfa();
  kprintf( " LCD: not implemented for this system\n" );
