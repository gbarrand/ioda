/*
 * $Id: kmacro.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kmacro.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.8  2004/12/06 08:31:08  barrand
 * *** empty log message ***
 *
 * Revision 1.7  2004/12/05 12:47:48  barrand
 * *** empty log message ***
 *
 * Revision 1.6  2003/11/04 09:54:38  barrand
 * *** empty log message ***
 *
 * Revision 1.5  2000/11/30 08:35:46  barrand
 * *** empty log message ***
 *
 * Revision 1.4  2000/05/28  04:19:34  barrand
 * *** empty log message ***
 *
 * Revision 1.3  1999/11/30  13:42:44  barrand
 * *** empty log message ***
 *
 * Revision 1.2  1999/11/08  11:32:56  barrand
 * *** empty log message ***
 *
...
{
  ku_alfa();
  kprintf( " *** Internal error: %s - Call KUIP maintenance\n", msg );
}
     ...
     char *source_line;
{
  ...
  if( kc_flags.appl_stream != NULL ) {
    ...
    while( p[0] == ' ' )
      p++;
    if( ku_strcasecmp( p, kc_flags.appl_exit ) != 0 )
      p = source_line;
     ...
     int which;
{
  ...
  for( pc = 0; pc < macro->ncode; pc++ ) {
    if( code[pc].op == op_label && code[pc].arg[n] != NULL
       && ku_strcasecmp( code[pc].arg[n], label ) == 0 ) {
      return pc;
    }
  }
     ...
     char *null2;
{
  ...
  if( value == -1 ) {
    ku_alfa();
    kprintf( " *** Invalid boolean expression in file %s\n",
           tos->file->path );
...
void store_variable( ExecStack* tos, const char* name, const char* value )
{
  ...
  if( tos != NULL ) {
    ...
    if( khash_lookup( tos->variables, name, &is_global ) != NULL
       && is_global ) {
      khash_insert( kc_alias.global_value, name, value, 0 );
    }
    khash_insert( tos->variables, name, value, is_global );
  }
  else
    khash_insert( kc_alias.global_value, name, value, 0 );
}
...
{
  int status;
  /*GB status = ku_exel( command_line );*/
  status = ku_exec_no_help( command_line ); /*GB*/
     ...
     char *list;
{
  ...
  for( p = strqtok( dlist ); p != NULL; p = strqtok( NULL ) ) {
    ...
    if( strcmp( p, "[*]" ) == 0 ) {
      int n = atoi( khash_lookup( tos->variables, "#", NULL ) );
      int i;
...
int op_fornext( tos, loop_var, list_var )
     ...
     char *list_var;
{
  char *list = strdup( khash_lookup( tos->variables, list_var, NULL ) );
     ...
     int in_parm;
{
  ...
  if( value[0] == '\'' && value[n-1] == '\'' ) {
    ...
    if( fixed ) {
      ...
      if( tos == NULL ) {
        kprintf( " *** Warning in EXEC command:\n" );
      }
      else {
        kprintf( " *** Warning in %s at line %s:\n",
               tos->file->path, tos->line_number );
      }
      kprintf( "     Old style use of quotes in macro argument fixed to %s\n",
             value );
    }
  }
     ...
     char *null2;
{
  ...
  if( parameter_list != NULL ) {
    ...
    for( tok = strqtok( line ); tok != NULL; tok = strqtok( NULL ) ) {
      ...
      if( strlen( name ) != len_variable( name ) ) {
        ku_alfa();
        kprintf( " *** Invalid name for macro parameter [%d] in file %s\n",
      ...
      khash_insert( tos->variables, name, value, 0 );
      sprintf( buf, "%d", ++npar );
      khash_insert( tos->variables, buf, value, 0 );
      free( value );
  ...
  if( tos->argline != NULL ) {
    ...
    for( tok = strqtok( line ); tok != NULL; tok = strqtok( NULL ) ) {
      ...
      if( n > 0 && tok[n] == '=' ) {
         ...
         */
        if( strcmp( tok + n + 1, "!" ) == 0 ) {
          value = khash_lookup( tos->variables, name, NULL );
          if( value == NULL )
            value = strdup( "!" );
          else
            value = strdup( value );
         ...
         */
        for( i = 0; i < npar; i++ ) {
          if( ku_strcasecmp( equiv[i], name ) == 0 ) {
            sprintf( buf, "%d", i + 1 );
            khash_insert( tos->variables, buf, value, 0 );
            break;
          }
        ...
        if( named ) {
          ...
          if( strcmp( tok, "!" ) == 0 ) {
            free( value );
            value = strdup( khash_lookup( tos->variables, name, NULL ) );
          }
          else {
            khash_insert( tos->variables, name, value, 0 );
          }
  ...
  sprintf( buf, "%d", narg );
  khash_insert( tos->variables, "#", buf, 0 );

  if( narg > 0 ) {
    line = strdup( khash_lookup( tos->variables, "1", NULL ) );
    for( i = 1; i < narg; i++ ) {
      sprintf( buf, "%d", i + 1 );
      line = mstr2cat( line, " ", khash_lookup( tos->variables, buf, NULL ) );
    }
    khash_insert( tos->variables, "*", line, 0 );
    free( line );
  }
  else {
    khash_insert( tos->variables, "*", "", 0 );
  }

  khash_insert( tos->variables, "@", "0", 1 );
  khash_insert( kc_alias.global_value, "@", "0", 0 );

  line = str3dup( tos->file->path, "#", tos->macro->name );
  khash_insert( tos->variables, "0", line, 0 );
  free( line );
...
int op_match( tos, variable, pattern_list )
     ...
     char *pattern_list;
{
  char *value = khash_lookup( tos->variables, variable, NULL );
     ...
     char *expression;
{
  ...
  if( status != 0 ) {
    ku_alfa();
    kprintf( " *** Invalid arithmetic expression in file %s\n",
	     tos->file->path );
    kprintf( "%s: %s\n", tos->line_number, tos->source_line );
    return -1;
  }
     ...
     char *prompt_expr;
{
  ...
  if( (value = khash_lookup( tos->variables, variable, NULL )) == NULL ) {
    value = "";
  }
  dfault = strdup( value );
     ...
     char *null2;
{
  ...
  for( i = 1; i < n; i++ ) {
    char *value;
    sprintf( buf, "%d", i + 1 );
    value = khash_lookup( tos->variables, buf, NULL );
    sprintf( buf, "%d", i );
  ...
  if( n > 0 ) {
    sprintf( buf, "%d", n );
    khash_remove( tos->variables, buf );
    sprintf( buf, "%d", n - 1 );
    store_variable( tos, "#", buf );
  }

  if( n > 1 ) {
    char *line = strdup( khash_lookup( tos->variables, "1", NULL ) );
    for( i = 1; i < n - 1; i++ ) {
      sprintf( buf, "%d", i + 1 );
      line = mstr2cat( line, " ", khash_lookup( tos->variables, buf, NULL ) );
    }
     ...
     char *line;
{
  ...
  if( ku_strcasecmp( tok1, "GO" ) == 0 && ku_strcasecmp( tok2, "TO" ) == 0 ) {
    tok1 = "GOTO";
    tok2 = strqtok( NULL );
  }


  if( ku_strcasecmp( tok1, "LET" ) == 0 ) {
    tok1 = tok2;
    tok2 = strqtok( NULL );
  }
  ...
  if( strchr( tok1 + 1, '=' ) != NULL
  ...
  else if( ku_strcasecmp( tok1, "IF" ) == 0 ) {
    n = strlen( line );

    if( ku_strcasecmp( line + n - 5, " THEN" ) == 0 ) {
      ...
      if( (p = strrchr( tokline, ' ' )) != NULL ) {
        ...
        *p = '\0';
        ...
        if( (p = strrchr( tokline, ' ' )) != NULL ) {
          ...
          else if( ku_strcasecmp( p + 1, "GOTO" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "ELSEIF" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "ELSE" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "ENDIF" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "CASE" ) == 0 ) {

    n = strlen( line );

    if( ku_strcasecmp( line + n - 3, " IN" ) != 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "ENDCASE" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "DO" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "ENDDO" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "FOR" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "ENDFOR" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "REPEAT" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "UNTIL" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "WHILE" ) == 0 ) {
    ...
    if( tok2 == NULL ) {
      ...
      n = strlen( line );
      if( ku_strcasecmp( line + n - 3, " DO" ) != 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "ENDWHILE" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "BREAKL" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "NEXTL" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "GOTO" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "MACRO" ) == 0 ) {
    ...
    else if( ku_strcasecmp( p, "GOTO" ) == 0
            || ( ku_strcasecmp( p, "GO" ) == 0 && (p = strqtok( NULL )) != NULL
                && ku_strcasecmp( p, "TO" ) == 0 ) ) {
    ...
    else if( ku_strcasecmp( p, "CONTINUE" ) == 0 ) {
      stmt = ON_ERROR_CONTINUE;
    }
    else if( ku_strcasecmp( p, "EXITM" ) == 0 ) {
      stmt = ON_ERROR_EXITM;
      if( (p = strqtok( NULL )) != NULL ) {
        stmt_arg1 = strdup( p );
      }
    }
    else if( ku_strcasecmp( p, "STOPM" ) == 0 ) {
      stmt = ON_ERROR_STOPM;
    }
    else {
  ...
  else if( ku_strcasecmp( tok1, "READ" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "SHIFT" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "EXITM" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "RETURN" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "STOPM" ) == 0 ) {
  ...
  else if( ku_strcasecmp( tok1, "ENDKUMAC" ) == 0 ) {
    ...
    stmt = EOF_REACHED;
          ...
          && ku_strcasecmp( cmd->path, "/KUIP/SET_SHOW/APPLICATION" ) == 0 ) {
  ...
  do {
    ...
    case APPLICATION_Statement:
      ...
      while( (p = read_line( stream, macro )) != NULL ) {
        while( p[0] == ' ' )
          p++;
        if( ku_strcasecmp( p, stmt_arg2 ) == 0 ) {
          break;
        }
      }
      if( p == NULL ) {
     ...
     char *path;
{
  ...
  if( compile_macros( file ) == 0 ) {
    ...
    khash_insert( kumac_table, path, file, 0 );
     ...
     char *argline;
{
  if( exec_stack_ptr < MAX_EXEC_STACK ) {
    ...
    tos->variables = khash_create( MAX_VARIABLE_TABLE );
    tos->on_error_stmt = ON_ERROR_CONTINUE;
    tos->on_error_arg = NULL;
    ...
    if( --exec_stack_ptr > 0 ) {
      ...
      if( (n = khash_entries( tos->variables )) > 0 ) {
        HashArray *values = khash_array( tos->variables );
        int i;
        ...
        for( i = 0; i < n; i++ ) {
          if( values[i].tag ) {
            ...
            if( khash_lookup( tos->variables, name, &is_global ) != NULL
               && is_global ) {
              char *value = (char*)khash_lookup( kc_alias.global_value,
                                               name, NULL );
              if( value == NULL ) {
                /* global has been deleted, delete local as well */
                khash_remove( tos->variables, name );
              }
              else {
     ...
     */
    {
      ...
      if( !needed ) {
        khash_remove( kumac_table, exec_stack[exec_stack_ptr].file->path );
    ...
    kprintf( " *** MACRO/EXEC: too many nested EXECs\n" );
    return KUMAC_UNWIND;
  }
}
...
int kxmexec()
{
  ...
  if( !kc_flags.in_macro ) {
    ...
    if( kumac_table == NULL ) {
      kumac_table = khash_create( MAX_KUMAC_TABLE );
      khash_config( kumac_table, "struct" );
      orphan_table = khash_create( MAX_KUMAC_TABLE );
      khash_config( orphan_table, "struct" );
#ifdef DBMALLOC
    ...
    else if( (n = khash_entries( kumac_table )) > 0 ) {
      HashArray *kumacs = khash_array( kumac_table );

      for( i = 0; i < n; i++ ) {
        file = kumacs[i].value;
        if( checksum( file->path ) != file->stamp ) {
          /* file was modified since the last compilation */
          khash_remove( kumac_table, kumacs[i].name );
          free_kumac( file );
        }
      }
      free( (char*)kumacs );
    ...
    if( (n = khash_entries( orphan_table )) > 0 ) {
      HashArray *kumacs = khash_array( orphan_table );

      for( i = 0; i < n; i++ ) {
        file = kumacs[i].value;
        khash_remove( orphan_table, kumacs[i].name );
        free_kumac( file );
      }
      free( (char*)kumacs );
    }
  ...
  if( kc_flags.in_macro && macro_name == NULL ) {
    ...
    for( macro = file->macros; macro != NULL; macro = macro->next ) {
      if( ku_strcasecmp( macro->name, kumac_name ) == 0 ) {
        break;
      }
    }
  }
  ...
  if( macro == NULL ) {
    ...
    if ( kumac_name[0] != '-' ) {
        ...
        if(!path) { /* G.Barrand */
          path = fsearch( ku_fcase(kumac_name), "kumac", kc_flags.macro_path );
        }
    }
    ...
    if( path == NULL ) {
      kprintf( " *** MACRO/EXEC: macro file %s not found\n", kumac_name );
    }
    else {
#if defined(UNIX) && !defined(MSDOS)
      khash_config( kumac_table, kc_flags.keep_fcase ? "respect" : "ignore" );
#endif
      if( (file = khash_lookup( kumac_table, path, NULL )) != NULL ) {
        ...
        if( checksum( file->path ) != file->stamp ) {
          ...
          if( needed ) {
            ...
            khash_insert( orphan_table, buf, file, 0 );
          }
          else {
            free_kumac( file );
          }
      ...
      if( file != NULL
        ...
        if( macro_name == NULL ) {
          ...
          for( macro = file->macros; macro != NULL; macro = macro->next ) {
            if( ku_strcasecmp( macro->name, macro_name ) == 0 ) {
              break;
            }
          }
        }
  ...
  if( macro == NULL ) {
    kprintf( " *** MACRO/EXEC: cannot execute %s\n", kumac_spec );
    return KUMAC_UNWIND;
  }
  else {
...
int ku_qmac( const char *mname )
{
  if( !kc_flags.in_macro ) {
    ...
    for( macro = file->macros; macro != NULL; macro = macro->next ) {
      if( ku_strcasecmp( macro->name, mname ) == 0 ) {
        return 1;
      }
    }
  }
...
/* { */
  ...
  if( (p = strchr( line, '=' )) != NULL ) {
    ...
    *p = '\0';
    khash_insert( kc_alias.var_table, line, value, 0 );
    free( value );
    IQUEST(1) = 0;
  }
  else {
...
int kxgcreate()
{
  ...
  khash_insert( kc_alias.global_value, name, value, 0 );
  khash_remove( kc_alias.global_text, name );
  if( text[0] != '\0' )
    khash_insert( kc_alias.global_text, name, text, 0 );

  if( kc_flags.in_macro ) {
    ExecStack *tos = &exec_stack[exec_stack_ptr-1];
    khash_insert( tos->variables, name, value, 1 );
  }
...
int kxgimport()
{
  if( kc_flags.in_macro ) {
    ...
    while( npar-- > 0 ) {
      ...
      int n = khash_entries( kc_alias.global_value );

      if( n > 0 ) {
        ExecStack *tos = &exec_stack[exec_stack_ptr-1];
        HashArray *values = khash_array( kc_alias.global_value );
        int i;
    ...
    if( n > 0 ) {
      ...
      for( i = 0; i < n; i++ ) {
        ...
        if( strcmp( name, "@" ) != 0 && ku_match( name, pattern, 1 ) ) {
          khash_remove( kc_alias.global_value, name );
          khash_remove( kc_alias.global_text, name );
        }
      }
      free( (char*)values );
    ...
    if( n > 0 ) {
      ...
      for( i = 0; i < n; i++ ) {
        ...
        if( ku_match( name, pattern, 1 ) ) {
          ...
          char *text = khash_lookup( kc_alias.global_text, name, NULL );
