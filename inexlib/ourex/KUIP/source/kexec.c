/*
 * $Id: kexec.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kexec.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.16  2009/07/06 08:52:14  barrand
 * *** empty log message ***
 *
 * Revision 1.15  2009/07/02 18:33:00  barrand
 * *** empty log message ***
 *
 * Revision 1.14  2009/06/29 16:27:30  barrand
 * *** empty log message ***
 *
 * Revision 1.13  2009/06/17 09:28:29  barrand
 * *** empty log message ***
 *
 * Revision 1.12  2005/09/19 07:14:32  barrand
 * *** empty log message ***
 *
 * Revision 1.11  2004/12/23 14:54:43  barrand
 * *** empty log message ***
 *
 * Revision 1.10  2004/09/03 14:36:23  barrand
 * *** empty log message ***
 *
 * Revision 1.9  2004/08/01 14:25:57  barrand
 * *** empty log message ***
 *
 * Revision 1.8  2004/08/01 14:22:53  barrand
 * *** empty log message ***
 *
 * Revision 1.7  2003/11/04 09:54:37  barrand
 * *** empty log message ***
 *
 * Revision 1.6  2000/05/28 04:19:33  barrand
 * *** empty log message ***
 *
 * Revision 1.5  1999/11/30  13:42:43  barrand
 * *** empty log message ***
 *
 * Revision 1.4  1999/11/08  11:25:19  barrand
 * *** empty log message ***
 *
 * Revision 1.3  1999/06/23  09:43:34  barrand
 * *** empty log message ***
 *
 * Revision 1.2  1999/06/22  15:30:14  barrand
 * *** empty log message ***
 *
 ...
 *
 */
/*CMZ :  2.07/04 23/05/95  15.13.57  by  N.Cremel*/
...
int kxmdefaults()
{
  ...
  if( strcmp( path, "?" ) == 0 ) {
    ...
    if( kc_flags.macro_path == NULL ) {
      kprintf( " *** No macro search path defined\n" );
    }
    else {
      kprintf( " Macro search path: %s\n", kc_flags.macro_path );
    }
  }
  ...
  if( strcmp( mdef, "A" ) == 0 || strcmp( mdef, "AUTO" ) == 0 ) {
    ...
    ku_alfa();
    kprintf( " Macro search order is %s\n", value );
  }
                    ...
                    int (*display_panel)() )
{
  ...
  if( !kc_flags.in_macro
    ...
    if( ku_strcasecmp( cmd_string, kc_flags.appl_exit ) != 0 ) {
                    ...
                    int block_panel )
{
  ...
  char *p;
  int status = 0; /*GB = 0*/
  /*kprintf("debug : exec_single_cmd :%s|\n",cmd_line);*/
  ...
  while( kc_flags.temp_vectors > 0 ) {
    char vname[16];
    sprintf( vname, "?SIG%d", kc_flags.temp_vectors-- );
    /*GB F77_CALL_C(Kuvdel,vname,strlen(vname));*/
  }
...
int kxmdata()
{
  ...
  if( stream == NULL ) {
    kprintf( " *** MACRO/DATA: cannot open file %s\n", appl_exit );
    return 1;
  }
  while( ku_read( appl_luno, line, 255 ) >= 0
        && ku_strcasecmp( appl_exit, line ) != 0 ) {
    fprintf( stream, "%s\n", line );
  }
  fclose( stream );
...
void ku_cmdl( const char *template )
{
  if( template[0] == '\0' ) {
    ...
    if( kc_value.set_command == NULL )
      kprintf( " COMMAND line not defined\n" );
    else
      kprintf( " COMMAND line is : %s\n", kc_value.set_command );
  }
                  ...
                  int block_panel )
{
  ...
  if( ku_strcasecmp( cmd->name, "EXEC" ) == 0 ) {
    /*
    ...
    while( 1 ) {
      ...
      if( len_argv == 1 && argv[0] == '!' && argc < cmd->total ) {
        ...
        for( i = 0; i < cmd->total; i++ ) {
          ...
          if( len >= par->abbrev && len <= strlen( par->name )
             && ku_strncasecmp( argv, par->name, len ) == 0 ) {
      ...
      if( argc < cmd->total ) {
        ...
        if( (par->flags & KmFLAG_CONST) != 0
          ...
          kprintf( " *** Parameter %s of %s is defined as a constant!\n",
                 par->name, cmd->name );
          free( cmd->argv[argc] );
  ...
  for( i = 0; i < cmd->mandatory; i++ ) {
    if( i >= cmd->argc || cmd->argv[i] == NULL ) {
      if( want_prompt ) {
        ...
        ku_alfa();
        kprintf( " *** Mandatory parameter %s missing\n", cmd->par[i]->name );
        return 1;
      }
    }
  }
             ...
             clock_t user_time )
{
  ...
  ku_alfa();
  kprintf( " Real time %s:%s:%s, CP time %.3f\n",
     ...
     KmCommand *cmd;
{
  ...
  if( cmd->action_F != NULL ) {
    IQUEST(1) = 0;
    (*cmd->action_F)(kjmpaddr.action_data);
  }
  else if( cmd->action_C != NULL ) {
    IQUEST(1) = (*cmd->action_C)(kjmpaddr.action_data);
  }
  kc_flags.action_nesting--;
...
int call_action( KmCommand *cmd )
{
  ...
  /*GB (*kjmpaddr.disp_flush_C)();*/ /* eat up queued Motif events */
    (*kjmpaddr.disp_flush_C)(kjmpaddr.disp_flush_C_data); /*GB*/
...
char *next_value( const char *called_by )
{
  ...
  if( value == NULL ) {
    ku_alfa();
    kprintf( " *** Command %s requests undefined parameter #%d with %s\n",
...
void ku_spy( const char *flag )
{
  ...
  if( ku_strcasecmp( flag, "ON" ) == 0 ) {
    save_next_arg = next_arg;
  }
  else if( ku_strcasecmp( flag, "OFF" ) == 0 ) {
...
int ku_geti()
{
  ...
  while( 1 ) {
    ...
    if( ku_math( value, &dval ) == 0 ) {
      ...
      if( par != NULL ) {
        ...
        if( par->type != KmTYPE_INT ) {
          kprintf( " *** %s: KUGETI called for non-I parameter %s\n",
	    curr_cmd->path, par->name );
          ...
          if( par->range_count > 0 ) {
            ...
            if( !found ) {
              kprintf( " *** %s: %s=%d not in list of allowed values\n",
      ...
      kprintf( " *** %s: invalid integer number '%s'\n",
             curr_cmd->path, value );
      do_prompt = 1;
    }
    ...
    if( do_prompt ) {
      if(par==NULL) { /*GB*/
        kprintf( " *** %s: KUGETI can't prompt for parameter.\n",
           curr_cmd->path);
        free( value );
        return 0;
      }
    ...
    if( ku_math( value, &rval ) == 0 ) {
      ...
      if( par != NULL ) {
        ...
        if( par->type != KmTYPE_REAL ) {
          kprintf( " *** %s: KUGETR called for non-R parameter %s\n",
                 curr_cmd->path, par->name );
        }
        else {
          ...
          if( par->range_count > 0 ) {
            ...
            if( !found ) {
              kprintf( " *** %s: %s=%f not in list of allowed values\n",
      ...
      kprintf( " *** %s: invalid real number '%s'\n",
             curr_cmd->path, value );
      do_prompt = 1;
    }
...
char *ku_name() /*GB*/
{
  return curr_cmd->name;
}

char *ku_macro_path() /*GB*/
{
  return kc_flags.macro_path;
}

int ku_exec_no_help( const char *cmd_string ) /*GB*/
{
  /* Exec without interactive help */
  return exec_cmd_string( cmd_string, 0, kjmpaddr.disp_panel_C );
}

char** ku_getopts(const char* aOPT,int* aN) /*GB*/
{
  /* User have to do a free over the returned value. */
  int i;
  for( i = 0; i < curr_cmd->total; i++ ) {
    KmParameter* par = curr_cmd->par[i];
    if( par->type == KmTYPE_OPTION ) {
      if( strcmp(aOPT,par->name)==0) {
        int j;
        char** opts;
        if(par->range_count<=0) { *aN = 0; return 0;}
        opts = (char**)malloc( par->range_count * sizeof(char*) );
        for( j = 0; j < par->range_count; j++ ) opts[j] = par->range_value[j]; /* WARNING : not copied. */
        *aN = par->range_count;
        return opts;
      }
    }
