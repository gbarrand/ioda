/*
 * $Id: kmenu.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kmenu.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.10  2009/06/04 14:17:08  barrand
 * *** empty log message ***
 *
 * Revision 1.9  2009/06/04 12:50:40  barrand
 * *** empty log message ***
 *
 * Revision 1.8  2003/11/04 09:54:38  barrand
 * *** empty log message ***
 *
 * Revision 1.7  2000/11/30 08:35:46  barrand
 * *** empty log message ***
 *
 * Revision 1.6  2000/03/08  15:04:44  barrand
 * *** empty log message ***
 *
 * Revision 1.5  2000/03/08  14:49:14  barrand
 * *** empty log message ***
 *
 * Revision 1.4  1999/11/08  11:32:09  barrand
 * *** empty log message ***
 *
 * Revision 1.3  1999/07/01  09:22:50  barrand
 * *** empty log message ***
 *
 * Revision 1.2  1999/07/01  07:35:11  barrand
 * *** empty log message ***
 *
 ...
 *
 */
/*CMZ :  2.07/08 21/11/95  17.00.30  by  N.Cremel*/
...
int kxroot()
{
  ...
  if( strcmp( path, "?" ) == 0 ) {
    ...
    if( menu_list == NULL ) {
      ku_alfa();
      kprintf( " *** Unknown command path\n" );
      status = -1;
    }
    else {
      if( menu_list[1] != NULL ) {
        ku_alfa();
        kprintf( " *** Ambiguous command path\n" );
        status = -1;
      }
      else {
                        ...
                        const char *cmd_path )
{
  ...
  for( i = 0; i < ncmd; i++ ) {
    if( ku_strcasecmp( cmd_list[i]->path, cmd_path ) == 0 ) {
      return cmd_list[i];
    }
    if( ku_strcasecmp( cmd_list[i]->name, cmd_name ) == 0 ) {
  ...
  if( (menu_list = find_menu_list( NULL, top_menu, menu_path )) != NULL ) {
    ...
    for( i = 0; menu_list[i] != NULL; i++ ) {
      ...
      for( cmd = menu_list[i]->cmds; cmd != NULL; cmd = cmd->next ) {
        ...
        if( ku_strncasecmp( cmd->name, cmd_name, cmd_len ) == 0 ) {
          /* add command to list */
          int n = 0;
                    ...
                    const char *path )
{
  ...
  if( cmd_list == NULL ) {
    ...
    int i;
    kprintf( " *** Ambiguous command %s. Possible commands are :\n", path );
    kprintf( "\n");
    for( i = 0; cmd_list[i] != NULL; i++ )
      kprintf( " %s\n", cmd_list[i]->path );
    kprintf( "\n");
                        ...
                        const char *menu_path )
{
  ...
  if( ku_strncasecmp( top_menu->name, head, head_len ) == 0 ) {
...
int kxmanual()
{
  ...
  if( strcmp( docsys_name, "SGML" ) == 0 ) {
    kprintf( " *** SGML not supported anymore. Please use LaTeX instead.\n" );
    return -1;
  }
  ...
  if( strcmp( path, "/" ) == 0 ) {
    ...
    if( cmd == NULL && menu_list == NULL ) {
      ...
      char *value = (char*)khash_lookup( kc_alias.cmd_table, path, NULL );
      if( value == NULL ) {
  ...
  case DOCSYS_LATEX:
     if( encapsulate ) {
      fputs( "\
\\documentstyle[a4,twoside]{report}\n\
%\\makeindex\n\
...
int km_strempty(char*); /*G.Barrand : clang*/
...
int kxfind()
{
  ...
  if( npar == 1 ) {
    ...
    if ( keyw_expr == NULL || km_strempty( keyw_expr ) ) {
      if(keyw_expr) free(keyw_expr); /*GB*/
      return 0;
    }
...
/* kprintf ("*** Top menu : %s\n", flis_name[nval]); */
	    flis_cmd[nval] = NULL; /*GB*/
	    flis_guidance[nval] = NULL; /*GB*/
            flis_flag[nval] = 3;
            fprl_flag[nval] = 0;
    ...
    for (j = 0; j < nkey; j++) {
        ...
        /* Loop on all commands from cmd_list */
	      ...
	      flis_path[nval] = NULL; /*GB*/
/* kprintf ("*** Real command : %s\n", flis_name[nval]); */
...
/* kprintf ("*** Keyword for command : %s\n", flis_name[nval]); */
    ...
    case 0:  /* No command associated to that keyword(s) */
      if (!opt || nkey > 0)
          kprintf("*** Cannot find any command associated to keyword(s): %s\n",
                  expr);
      break;
...
associated to the keyword(s)\n\n       ", expr);
      ...
      /* Command line interface */
	   ...
	   if(listFrom) free(listFrom); /*GB*/
           ...
           for (i=0; i<nval; i++) {
               ...
               if( fprl_flag[i] == 0)
                   ...
                   flag = " * ";
               kprintf( "%s:%s%-12s  ", help_label( i+1 ), flag, flis_name[i]);
               if( strlen( flis_name[i] ) > 12 ) {
                 kprintf( "\n");
                 kprintf( "%20s", "" );
	       }
               kprintf("%s",flis_guidance[i]);
           }
        ...
        if (nkey > 0) {
           kprintf( "______\n");
           kprintf( "%s:   %-12s  ", help_label( 0 ), "");
           kprintf("%s", "Top level menu");
	   fflush( stdout ); /* or the line is delayed on linux?! */
        ...
        } else if ( ku_strcasecmp( line, "Q" ) != 0 ) {
           nl = atoi( line );
           if (nl > 0) {
               char cmd[512];
    ...
    for (j = 0; j < nkey; j++) {if(keyw[j]) free(keyw[j]);} /*GB*/
    free (keyw);
    realloc_find ( -1 ); 
    free(menu_list); /*GB*/
}
...
void realloc_find( int nval )
{
    if (nval < 0 ) {
      int count;
      for(count=0;count<flis_size;count++) { /*GB*/
	if(flis_path[count]) free (flis_path[count]); /*GB*/
	if(flis_name[count]) free (flis_name[count]); /*GB*/
	if(flis_cmd[count]) free (flis_cmd[count]); /*GB*/
	if(flis_guidance[count]) free (flis_guidance[count]); /*GB*/
      }
...
void print_help_line( int nl, char *command, char *message, int fl )
{
    ...
    kprintf( "%s:%s%-12s  ", help_label( nl ), flag, command);
  ...
  while( 1 ) {
    ...
    for( cmd = top_menu->cmds; cmd != NULL; cmd = cmd->next ) {
      ...
      if( cmd->action_F == NULL && cmd->action_C == NULL )
        ...
        flag = " * ";
      kprintf( "%s:%s%-12s  ", help_label( ++ncmds ), flag, cmd->name );
    ...
    nmenus = ncmds;
    for( menu = top_menu->down; menu != NULL; menu = menu->next ) {
      kprintf( "%s:   %-12s  ", help_label( ++nmenus ), menu->name );
        ...
        if( n > 0 ) {
          if( n <= ncmds ) {
            ...
            while( 1 ) {
              ...
              if( !hidden && ku_strcasecmp( line, "X" ) == 0 ) {
                char arg_prmpt[80];
                kprintf( "Add parameters or just <CR> to the command line\n" );
                kprintf( "('#'=cancel execution, '?'=help) :\n\n" );
                   ...
                   int docsys )
{
  ...
  if( cmd->user_help_C != NULL ) {
    ...
    /*GB (*cmd->user_help_F)();*/
    (*cmd->user_help_F)(kjmpaddr.action_data); /*GB*/
...
/*GB*/
char* ku_help(int* lun) {
  *lun = kc_flags.uhlp_luno;
  return user_help_name;
}

/*GB*/
char* ku_uhlp_file() {
  return ku_fcase( kc_flags.uhlp_file );
}
                      ...
                      const char *called_by )
{
  ...
  if( cmd != NULL && ku_strcasecmp( cmd->name, cmd_name ) == 0 )
    return cmd;

  if( curr_menu != NULL ) {
    for( cmd = curr_menu->cmds; cmd != NULL; cmd = cmd->next ) {
      if( ku_strcasecmp( cmd->name, cmd_name ) == 0 )
        return cmd;
    }
  }
...
/* { */
  ...
  if( ku_strcasecmp( opt, "C" ) == 0 ) {
  ...
  else if( ku_strcasecmp( opt, "SW" ) == 0 ) {
    /* switch to menu */
    char *menu_name = fstrdup( refpat, len_refpat );

    if( curr_cmd != NULL && ku_strcasecmp( menu_name, curr_cmd->name ) == 0 ) {
      /*
      ...
      if( curr_menu->down == NULL ) {
        ...
        while( 1 ) {
          if( ku_strcasecmp( curr_menu->name, curr_cmd->name ) == 0 ) {
            break;
          }
    ...
    *cmd_name++ = '\0';
    ...
    for( cmd = menu->cmds; cmd->next != NULL; cmd = cmd->next ) {
      if( ku_strcasecmp( cmd->next->name, cmd_name ) == 0 ) {
        cmd->next = cmd->next->next;
        break;
      }
    }
  }
  else if( ku_strcasecmp( opt, "E" ) == 0 ) {
    /* end definition */
  }
  else {
    kprintf( " *** KUCMD: invalid option '%s'\n", opt );
  }
  free( opt );
}
  ...
  if( cmd != NULL ) {
    ...
    if( ku_strcasecmp( opt, "S" ) == 0 ) {
      ...
      *nlines = cmd->nguidance;
      ...
      kprintf( " *** KUGUID: invalid option '%s'\n", opt );
    }
    free( opt );
  }
  free( path );
    ...
    if( ku_strcasecmp( opt, "S" ) == 0 ) {
      ...
      for( i = 1; i < len_type; i++ ) {
        ...
        case ' ':
          ...
          break;
        default:
          kprintf( " *** KUPAR: invalid modifier '%c'\n", type[i] );
        }
      }
      ...
      kprintf( " *** KUPAR: invalid option '%s'\n", opt );
    }
    free( opt );
  }
  free( path );
...
/* { */
  ...
  if( cmd != NULL ) {
    ...
    for( i = 0; i < cmd->total; i++ ) {
      if( ku_strcasecmp( cmd->par[i]->name, par_name ) == 0 ) {
        par = cmd->par[i];
        break;
      }
    }
    if( par == NULL ) {
      kprintf( " *** KUPVAL: command %s has no parameter named %s\n",
             cmd->path, par_name );
    }
    else {
      char *opt = fstrdup( chopt, len_chopt );
      ...
      if( opt[0] == '-' ) {
        ...
        if( par->type == KmTYPE_OPTION ) {
          ...
          for( n = 0; n < par->range_count; n++ ) {
            if( ku_strcasecmp( par->range_value[n], &opt[1] ) == 0 ) {
      ...
      else if( ku_strcasecmp( opt, "ABBREV" ) == 0 ) {
        par->abbrev = *ival;
      }
      else if( ku_strcasecmp( opt, "FLAGS" ) == 0 ) {
        par->flags |= *ival;
      }
      else if( ku_strcasecmp( opt, "LOOP" ) == 0 ) {
        cmd->list_par = i + 1;
      }
      ...
      if( wrong )
        kprintf( " *** KUPVAL: invalid option '%s' for %s %s\n",
