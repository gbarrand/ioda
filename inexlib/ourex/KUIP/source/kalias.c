/*
 * $Id: kalias.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kalias.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.3  2003/11/04 09:54:37  barrand
 * *** empty log message ***
 *
 * Revision 1.2  1999/11/08 11:23:24  barrand
 * *** empty log message ***
 *
...
int kxacreate()
{
  ...
  if( strchr( option, 'C' ) != NULL ) {
    char *alias = khash_lookup( kc_alias.arg_table, name, NULL );
    if( alias != NULL ) {
      ku_alfa();
      kprintf( " *** Argument alias '%s' superseded by command alias\n", 
	       name );
      khash_remove( kc_alias.arg_table, name );
    }
    khash_insert( kc_alias.cmd_table, name, value, 0 );
  }
  else {
    char *alias = khash_lookup( kc_alias.cmd_table, name, NULL );
    if( alias != NULL ) {
      ku_alfa();
      kprintf( " *** Command alias '%s' superseded by argument alias\n", 
	       name );
      khash_remove( kc_alias.cmd_table, name );
    }
    /* apply alias translations in value unless inhibited by option N */
    if( strchr( option, 'N' ) == NULL ) {
      char *p;
      if( (p = subst_arg_alias( value )) != NULL ) {
        khash_insert( kc_alias.arg_table, name, p, 0 );
        free( p );
        return 0;
      }
    ...
    khash_insert( kc_alias.arg_table, name, value, 0 );
  }
...
int kxatranslation()
{
  ...
  if( strcmp( option, "ON" ) == 0 )
    ...
    kprintf( " Current alias translation: %s\n",
...
int kxadelete()
{
  ...
  int n = khash_entries( kc_alias.cmd_table );

  if( n > 0 ) {
    HashArray *values = khash_array( kc_alias.cmd_table );
    int i;
  ...
  if( (n = khash_entries( kc_alias.arg_table )) > 0 ) {
    HashArray *values = khash_array( kc_alias.arg_table );
    int i;
...
int kxalist()
{
  ...
  for( k = 0; k < 2; k++ ) {
    int n = khash_entries( table[k] );
    if( n == 0 ) {
      kprintf( " No %s aliases defined.\n", header[k] );
    }
    else {
      HashArray *alias = khash_array( table[k] );
      int i;
     ...
     char *line;
{
  if( kc_alias.translate && kc_alias.substitutions > 0 ) {
    ...
    if( (p = khash_lookup( kc_alias.cmd_table, cmd, NULL )) != NULL
       || (p = khash_lookup( kc_alias.arg_table, cmd, NULL )) != NULL ) {
      ...
      if( --kc_alias.substitutions <= 0 ) {
        ku_alfa();
        kprintf( " *** Recursive command alias in %s\n", new_line );
      }
     ...
     char *line;
{
  ...
  if( kc_alias.translate
    ...
    ku_alfa();
    kprintf( " *** Recursive argument alias in %s\n", line );
    free( new_line );
  }
  return NULL;
}
...
char *var_value( const char *var )
{
  ...
  value = khash_lookup( kc_alias.var_table, name, NULL );
     ...
     char *line;
{
  ...
  if ( subst_calls > 100 ) {
     ku_alfa();
     kprintf(" Recursive variable assignment stopped in line \n");
     kprintf("%s\n",line);
     return(NULL);
  }
      ...
      if( len > 0 && p[len+1] != ':' ) {
        ...
        for( i = 0; i < (sizeof sysfun) / sizeof( struct sysfun ); i++ ) {
          ...
          if( len == strlen( sysfun[i].name )
             && ku_strncasecmp( p + 1, sysfun[i].name, len ) == 0 ) {

            switch( sysfun[i].code ) {

            case SysANUM:       /* number of aliases */
              value = malloc( 16 );
              sprintf( value, "%d", khash_entries( kc_alias.arg_table ) );
              break;
            ...
            case SysNUMVEC:
              value = malloc( 16 );
              /*GB sprintf( value, "%d", (int)Kcvect.NUMVEC - 1 );*/
              sprintf( value, "%d", 0); /*GB*/
              break;
        ...
        if( value == NULL && eval && p[len+1] == '(' ) {
          ...
          if( arglen > 0 ) {
            ...
            int is_eval = (ku_strncasecmp( p + 1, "EVAL", len ) == 0);
            int is_unquote = (ku_strncasecmp( p + 1, "UNQUOTE", len ) == 0);
...
char **ku_qenv()
{
  ...
  for( k = 0; k < 2; k++ ) {
    ...
    if( k == 0 ) {
      nalias = khash_entries( kc_alias.arg_table );
      alias = khash_array( kc_alias.arg_table );
      opt = "'";
    }
    else {
      nalias = khash_entries( kc_alias.cmd_table );
      alias = khash_array( kc_alias.cmd_table );
      opt = "' 'C'";
    }
