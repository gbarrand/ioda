/*
 * $Id: kuwhat.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kuwhat.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.16  2010/03/24 10:35:52  barrand
 * *** empty log message ***
 *
 * Revision 1.15  2009/11/18 13:59:25  barrand
 * *** empty log message ***
 *
 * Revision 1.14  2009/06/30 12:26:52  barrand
 * *** empty log message ***
 *
 * Revision 1.13  2009/06/30 08:49:14  barrand
 * *** empty log message ***
 *
 * Revision 1.12  2009/06/29 16:27:47  barrand
 * *** empty log message ***
 *
 * Revision 1.11  2009/06/17 09:28:21  barrand
 * *** empty log message ***
 *
 * Revision 1.10  2005/06/08 07:55:28  barrand
 * *** empty log message ***
 *
 * Revision 1.9  2004/12/23 14:54:26  barrand
 * *** empty log message ***
 *
 * Revision 1.8  2004/09/04 09:40:06  barrand
 * *** empty log message ***
 *
 * Revision 1.7  2004/09/03 14:33:55  barrand
 * *** empty log message ***
 *
 * Revision 1.6  2004/07/14 10:03:49  barrand
 * *** empty log message ***
 *
 * Revision 1.5  2003/09/10 09:25:23  barrand
 * *** empty log message ***
 *
 * Revision 1.4  2003/09/10 07:17:40  barrand
 * *** empty log message ***
 *
 * Revision 1.3  1999/12/10 08:34:27  barrand
 * *** empty log message ***
 *
 * Revision 1.2  1999/11/08  11:30:42  barrand
 * *** empty log message ***
 *
...
int kxnewpan()
{
  ...
  if (width == 0 || height == 0) {
      kprintf( " *** Invalid value for width (%d) or height (%d) \n",
              width, height );
      return 1;
  }
  ...
  while( !kc_flags.do_exit && !kc_flags.do_quit ) {
    ...
    if( kc_flags.in_application ) {
      ...
      if( cmd != NULL && kjmpaddr.user_input_F != NULL ) {
        kprintf("debug : what_loop : dummy:%s|\n",cmd);
 ...
 */
void ku_sigm(SigmaFunc fun,void* aData) {
  kjmpaddr.user_sigma_F = fun;
  kjmpaddr.user_sigma_F_data = aData;
}
     ...
     INTEGER *iflag;
{
  if( *iflag == 0 ) {
    ...
    *iflag = (kjmpaddr.user_sigma_F != NULL);
    ...
    kprintf("debug : Kisigm : disconnected.\n"); /*GB*/
    /* call SIGMA */
...
int kxexit()
{
  ...
  if( strcmp( cmd_name, "/KUIP/EXIT" ) == 0 ) {
    ...
    if( kjmpaddr.disp_exit_C != NULL )
      kc_flags.do_exit = (*kjmpaddr.disp_exit_C)(kjmpaddr.disp_exit_C_data); /* ask for confirmation */
    else
      kc_flags.do_exit = 1;
  ...
  if( strcmp( cmd_name, "/KUIP/QUIT" ) == 0 ) {
    ...
    if( kjmpaddr.disp_quit_C != NULL )
      kc_flags.do_quit = (*kjmpaddr.disp_quit_C)(kjmpaddr.disp_quit_C_data); /* ask for confirmation */
    else
      kc_flags.do_quit = 1;
...
char *subst_event( const char *cmd_string )
{
  ...
  if( cmd_string[1] == '!' ) {
    ...
    for( n = cmd_hist.count; n > 0; n-- ) {
      if( ku_strncasecmp( cmd_hist.lines[n-1], &cmd_string[1], len ) == 0 )
        break;
    }
  }

  if( n <= 0 || n > cmd_hist.count ) {
    kprintf( " *** Command %s... not in history buffer\n", cmd_string );
    return NULL;
  }
...
int kxstyle()
{
  ...
  if( strcmp( style, "?" ) == 0 ) {
    kprintf( " Current input style is '%s'.\n", style_name( kc_flags.style ) );
    return 0;
  }
  ...
  if( 1 ) {
    ...
    if( strcmp( style, filecase_on ) == 0 ) {
      kprintf( " %s '%s' is a deprecated feature.\n", ku_path(), filecase_on );
      kprintf( " Please use %s instead.\n", filecase_mixed );
      return ku_exel( filecase_mixed );
    }
    if( strcmp( style, filecase_off ) == 0 ) {
      kprintf( " %s '%s' is a deprecated feature.\n",ku_path(), filecase_off );
      kprintf( " Please use %s instead.\n", filecase_lower );
  ...
  default:
    kprintf( " *** Invalid style option '%s'\n", style );
    return -1;
  }
  ...
  if( (new_style & KmSTYLE_XM) == 0 && kc_flags.in_motif ) {
    kprintf( " *** Cannot change style in X/Motif\n - KUWHAT not called\n" );
    return -1;
  }
  if( (new_style & KmSTYLE_XM) != 0 && !kc_flags.in_motif ) {
    kprintf( " *** Cannot change style to X/Motif - KUWHAM not called\n" );
    return -1;
  }
...
int kxdollar()
{
  ...
  if( strcmp( which, "ON" ) == 0 ) {
    ...
    kprintf( " $var is substituted if defined as environment variable.\n" );
  }
  else {
    kprintf( " $var is not tried as being an environment variable.\n" );
  }
...
int kxfilecase()
{
  ...
  if( strcmp( fcase, "KEEP" ) == 0 ) {
    ...
    kprintf( " Filenames are NOT case converted.\n" );
  }
  else {
    kprintf( " Filenames ARE case converted.\n" );
  }
...
int kxrecallstyle()
{
  ...
  if( strcmp( style, "NONE" ) == 0 ) {
    ...
    if( kc_window.use_getline )
      style = styles[kgl_mask];
    else
      style = "NONE";
    kprintf( " Current recall style is %s\n", style );
    return 0;
  }
  ...
  if( kc_window.use_getline ) {
    /*kgl_config( "style", (kgl_mask >> 1) & 1 );*/
    /*kgl_config( "overwrite", kgl_mask & 1 );*/
    if( kjmpaddr.kgl_config!=NULL) { /*GB*/
      (*kjmpaddr.kgl_config)( "style", (kgl_mask >> 1) & 1 );
      (*kjmpaddr.kgl_config)( "overwrite", kgl_mask & 1 );
    }
  }
#endif
  return 0;
...
void reset_break()
{
  ...
  /*kgl_config( "noecho", 0 );*/
  if( kjmpaddr.kgl_config!=NULL) { /*GB*/
    (*kjmpaddr.kgl_config)( "erase", 0 );
    (*kjmpaddr.kgl_config)( "noecho", 0 );
  }
#endif
  ...
  if( kc_flags.in_macro ) {
    kc_flags.in_macro = 0;
    kprintf( " *** Macro execution stopped\n" );
  }
                 ...
                 char cont_char )
{
  ...
  if( kjmpaddr.disp_prompt_C != NULL ) {
    ...
    if( kc_window.use_no_echo )
      ...
      answer = (*kjmpaddr.disp_prompt_C)( prompt_string,kjmpaddr.disp_prompt_C_data );
      /*GB answer = (*kjmpaddr.disp_prompt_C)( prompt_string, NULL );*/
...
#pragma linkage(LNRDPAS,OS)
    ...
    if( kc_window.use_getline ) {
      ...
      if( kc_window.use_no_echo ) {
        /*kgl_config( "noecho", 1 );*/
        if( kjmpaddr.kgl_config!=NULL) { /*GB*/
          (*kjmpaddr.kgl_config)( "noecho", 1 );
        }
      }
      /*buf = kgl_getline( prompt )*/
      buf = NULL;
      if( kjmpaddr.kgl_getline!=NULL) { /*GB*/
        buf = (*kjmpaddr.kgl_getline)(prompt);
      }
      if( buf == NULL )
        eof = 1;
      else
...
# endif
  ...
  if( kc_window.use_getline && cont_char != '\0' )
    /*kgl_histadd( line );*/
    if( kjmpaddr.kgl_histadd!=NULL) { /*GB*/
      (*kjmpaddr.kgl_histadd)(line);
    }
#endif
              ...
              const char *dfault )
{
  ...
  if( dfault == NULL ) {
    ...
    /*GB char *prmt = str4dup( prompt, " (<CR>=", dfault, ")" );*/
    char *prmt = str4dup( prompt, " (<CR>=", dfault, ") " ); /*GB : add a trailing space.*/
            ...
            int ifault )
{
  ...
  while( 1 ) {
    ...
    if( line == NULL || line == dfault ) {
      ...
      kprintf( " *** Invalid integer number ***\n" );
    }
  }
}
...
/*G.Barrand : begin */
void ku_action_data(void* aData) {kjmpaddr.action_data = aData;}
void ku_prompt(char*(*aFunction)(char*,void*),void* aData) {
  kjmpaddr.disp_prompt_C = (CharFunc*)aFunction;
  kjmpaddr.disp_prompt_C_data = aData;
}
void ku_flush(int(*aFunction)(void*),void* aData) {
  kjmpaddr.disp_flush_C = (IntFunc*)aFunction;
  kjmpaddr.disp_flush_C_data = aData;
}
void ku_exit(int(*aFunction)(void*),void* aData) {
  kjmpaddr.disp_exit_C = (IntFunc*)aFunction;
  kjmpaddr.disp_exit_C_data = aData;
}
void ku_quit(int(*aFunction)(void*),void* aData) {
  kjmpaddr.disp_quit_C = (IntFunc*)aFunction;
  kjmpaddr.disp_quit_C_data = aData;
}
void ku_xvcreate(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_create_C = (IntFunc*)aFunction;
  kjmpaddr.vector_create_C_data = aData;
}
void ku_xvdelete(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_delete_C = (IntFunc*)aFunction;
  kjmpaddr.vector_delete_C_data = aData;
}
void ku_xvlist(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_list_C = (IntFunc*)aFunction;
  kjmpaddr.vector_list_C_data = aData;
}
void ku_xvcopy(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_copy_C = (IntFunc*)aFunction;
  kjmpaddr.vector_copy_C_data = aData;
}
void ku_xvinput(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_input_C = (IntFunc*)aFunction;
  kjmpaddr.vector_input_C_data = aData;
}
void ku_xvprint(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_print_C = (IntFunc*)aFunction;
  kjmpaddr.vector_print_C_data = aData;
}
void ku_xvread(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_read_C = (IntFunc*)aFunction;
  kjmpaddr.vector_read_C_data = aData;
}
void ku_xvwrite(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_write_C = (IntFunc*)aFunction;
  kjmpaddr.vector_write_C_data = aData;
}
void ku_xvoperation(int(*aFunction)(void*),void* aData) {
  kjmpaddr.vector_operation_C = (IntFunc*)aFunction;
  kjmpaddr.vector_operation_C_data = aData;
}
void ku_xvfind(int(*aFunction)(const char*,void*),void* aData) {
  kjmpaddr.vector_find_C = (IntFunc*)aFunction;
  kjmpaddr.vector_find_C_data = aData;
}
void ku_xvvalue(int(*aFunction)(const char*,double*,void*),void* aData) {
  kjmpaddr.vector_value_C = (IntFunc*)aFunction;
  kjmpaddr.vector_value_C_data = aData;
}

void ku_xgl_setwidth(void(*aFunction)()) {
  kjmpaddr.kgl_setwidth = (VoidFunc*)aFunction;
}
void ku_xgl_config(void(*aFunction)(const char*,int)) {
  kjmpaddr.kgl_config = (VoidFunc*)aFunction;
}
void ku_xgl_getline(char*(*aFunction)(const char*)) {
  kjmpaddr.kgl_getline = (CharFunc*)aFunction;
}
void ku_xgl_histadd(void(*aFunction)(const char*)) {
  kjmpaddr.kgl_histadd = (VoidFunc*)aFunction;
}
