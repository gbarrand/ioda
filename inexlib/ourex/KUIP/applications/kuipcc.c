/*
 * $Id: kuipcc.c,v 1.1.1.1 2013/07/18 11:08:49 barrand Exp $
 *
 * $Log: kuipcc.c,v $
 * Revision 1.1.1.1  2013/07/18 11:08:49  barrand
 *
 *
 * Revision 1.3  2006/10/02 15:31:58  barrand
 * *** empty log message ***
 *
 * Revision 1.2  2000/06/30 11:01:08  barrand
 * *** empty log message ***
 *
 * Revision 1.1.1.1  1999/05/25  13:19:04  barrand
 * KUIP
 *
     ...
     char **argv;
{
  ...
  while( iarg < argc && argv[iarg][0] == '-' && argv[iarg][1] != '\0' ) {
    if( ku_strcasecmp( argv[iarg], "-split" ) == 0 ) {
      split = 1;
    }
    else if( ku_strcasecmp( argv[iarg], "-c" ) == 0 ) {
      cmode = 1;
      explicit = 1;
    }
    else if( ku_strcasecmp( argv[iarg], "-f" ) == 0 ) {
      cmode = 0;
      explicit = 1;
    }
    else {
  ...
  if( argc > iarg ) {
    ...
    /*GB    fprintf( stderr, "Enter CDF input file name: " );
    gets( ifile );
    fprintf( stderr, "Enter definition output file name: " );
    gets( ofile );
    */
    fprintf( stderr, "Give a CDF input file name as argument :\n" );
    fprintf( stderr, "  kuipcc -c <cdf file>\n" );
    exit(1);
  }
  ...
  if( !explicit ) {
    ...
    cmode = (ku_strcasecmp( &ofile[len-2], ".c" ) == 0);
  }
  fmode = !cmode;
  ...
  while( read_line() > 0 ) {
    ...
    if( ku_strncasecmp( word[0], ">A", 2 ) == 0 ) {
      check_abbrev( word[0], ">Action" );
      proc_action( ACT_ION );
    }
    else if( ku_strcasecmp( word[0], ">Browse" ) == 0 ) {
      proc_browse();
    }
    else if( ku_strcasecmp( word[0], ">Button" ) == 0 ) {
      proc_button();
    }
    else if( ku_strcasecmp( word[0], ">Class" ) == 0 ) {
      proc_class();
    }
    else if( ku_strncasecmp( word[0], ">C", 2 ) == 0 ) {
      check_abbrev( word[0], ">Command" );
      proc_command( COM_MAND );
    }
    else if( ku_strcasecmp( word[0], ">Fortran" ) == 0 ) {
      proc_fortran();
    }
    else if( ku_strcasecmp( word[0], ">Graphics" ) == 0 ) {
      proc_graphics();
    }
    else if( ku_strncasecmp( word[0], ">G", 2 ) == 0 ) {
      check_abbrev( word[0], ">Guidance" );
      proc_guidance();
    }
    else if( ku_strncasecmp( word[0], ">K", 2 ) == 0 ) {
      check_abbrev( word[0], ">Keyword" );
      proc_keyword();
    }
    else if( ku_strncasecmp( word[0], ">L", 2 ) == 0 ) {
      check_abbrev( word[0], ">Link" );
      proc_hlink();
    }
    else if( ku_strncasecmp( word[0], ">H", 2 ) == 0 ) {
      check_abbrev( word[0], ">Help_item" );
      proc_command( COM_HELP );
    }
    else if( ku_strcasecmp( word[0], ">Icon_bitmaps" ) == 0 ) {
      proc_icon_bitmaps();
    }
    else if( ku_strcasecmp( word[0], ">Motif_customize" ) == 0 ) {
      proc_motif_customize();
    }
    else if( ku_strncasecmp( word[0], ">M", 2 ) == 0 ) {
      check_abbrev( word[0], ">Menu" );
      proc_menu();
    }
    else if( ku_strncasecmp( word[0], ">N", 2 ) == 0 ) {
      check_abbrev( word[0], ">Name" );
      proc_name();
    }
    else if( ku_strncasecmp( word[0], ">P", 2 ) == 0 ) {
      check_abbrev( word[0], ">Parameters" );
      proc_parameters();
    }
    else if( ku_strncasecmp( word[0], ">U", 2 ) == 0 ) {
...
void proc_button()
{
  ...
  if( word[7] != NULL ) {
      if( ku_strcasecmp( word[7], "NS" ) == 0 )
          but->flags = KmButtNonSensitive;
      else if( ku_strcasecmp( word[7], "TS" ) == 0 )
          but->flags = KmButtToggleSensitive;
      else if( ku_strcasecmp( word[7], "BR" ) == 0 )
  ...
  while( read_line() > 0 ) {
    ...
    *p++ = '\0';
    ...
    for( n = 0; n < nbytes && read_line() > 0; n += 12 ) {
      ...
      for( i = 0; i < m; i++ )
        icn->bitmap[n+i] = (char)strtol( word[i], NULL, 0 ); /*GB cast*/
    }
  }
}
...
void proc_parameters()
{
  ...
  while( read_line() > 0 ) {
    ...
    if( star == NULL )
      ...
      /*GB strcpy( star, star + 1 );*/
      strcpy( tmp, star + 1 ); /*GB*/
      strcpy( star, tmp);      /*GB*/
    }
    ...
    if( ku_strcasecmp( word[2], "C" ) == 0 )
      par->type = KmTYPE_CHAR;
    else if( ku_strcasecmp( word[2], "I" ) == 0 )
      par->type = KmTYPE_INT;
    else if( ku_strcasecmp( word[2], "R" ) == 0 )
    ...
    if( ku_strncasecmp( word[0], "CHOPT", 5 ) == 0
       || ku_strncasecmp( word[0], "OPTION", 6 ) == 0 )
    ...
    for( i = 3; i < nword; i++ ) {
      ...
      if( ku_strcasecmp( tag, "D" ) == 0 ) { /* default */
        if( value == NULL )
          warning( "Default value missing" );
        else
          par->dfault = strdup( value );
      }
      else if( ku_strcasecmp( tag, "R" ) == 0 ) { /* range */
        if( value == NULL )
      ...
      else if( ku_strcasecmp( tag, "SELECT" ) == 0 ) {
        only_cmode( "SELECT" );
        if( value == NULL )
      ...
      else if( ku_strcasecmp( tag, "SEPARATE" ) == 0 ) {
        if( value != NULL )
          warning( "Value in SEPARATE definition ignored" );
        par->flags |= KmFLAG_SEPARATE;
      }
      else if( ku_strcasecmp( tag, "SLIDER" ) == 0 ) {
        only_cmode( "SLIDER" );
        if( value == NULL )
      ...
      else if( ku_strcasecmp( tag, "VARARG" ) == 0 ) {
        if( value != NULL )
          warning( "Value in VARARG definition ignored" );
        par->flags |= KmFLAG_VARARG;
      }
      else if( ku_strcasecmp( tag, "WIDTH" ) == 0 ) {
        only_cmode( "WIDTH" );
        if( value == NULL )
    ...
    if( par->type == KmTYPE_OPTION ) {
      ...
      while( read_line() > 0 ) {
        ...
        if( full_line[0] != '-' ) {
          ...
          for( n = 0; n < par->range_count; n++ ) {
            if( ku_strcasecmp( par->range_value[n], value ) == 0 )
              break;
          }
      ...
      if( par->dfault != NULL && par->range_count > 0
        ...
        for( n = 0; n < par->range_count; n++ ) {
          if( ku_strcasecmp( par->range_value[n], par->dfault ) == 0 )
            break;
                ...
                const char *path )
{
  ...
  if( menu->down == NULL )
    ...
    for( m = menu->down; m != NULL; m = m->next ) {
      if( ku_strcasecmp( m->name, name ) == 0 )
        return m;
      prev = m;
    }
...
char *new_external_F( const char *name )
{
  ...
  for( i = 0; i < nexternal_F; i++ ) {
    if( ku_strcasecmp( external_F[i], name ) == 0 )
      return uname;
  }
                     ...
                     int type )
{
  ...
  for( i = 0; i < nexternal_C; i++ ) {
    if( ku_strcasecmp( external_C[i], name ) == 0 )
      return name;
  }
    ...
    for( i = 0; i < group->groups; i++ ) {
      ...
      for( j = 0; j < group->count[i]; j++ ) {
        ...
        for( k = 0; k < par->range_count; k++ ) {
          if( ku_strcasecmp( par->range_value[k], value[j] ) == 0 ) {
            if( pindex[k] != 0 ) {
                ...
                BrActTag tag )
{
  ...
  if( accel != NULL && ku_strncasecmp( accel, "S=0", 3 ) == 0 ) {
    accel += 3;
  }
  else {
