 ...
 *  Update Date:      $Date: 2013/04/16 19:23:56 $
 ...
 private:
                       ...
                       const int degree)
    {
      if (DiColorPixelTemplate<T>::Init(pixel)) //G.Barrand
        {
            if (degree == 90)
	      DiRotateTemplate<T>::rotateRight(pixel, this->Data); //G.Barrand
            else if (degree == 180)
	      DiRotateTemplate<T>::rotateTopDown(pixel, this->Data); //G.Barrand
            else  if (degree == 270)
	      DiRotateTemplate<T>::rotateLeft(pixel, this->Data); //G.Barrand
        }
    }
};
