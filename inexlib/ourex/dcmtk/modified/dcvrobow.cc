 ...
 *  Update Date:      $Date: 2013/07/02 07:16:20 $
                                               ...
                                               OFBool /*normalize*/)
{
    if (getTag().getEVR() == EVR_OW || getTag().getEVR() == EVR_lt)
    {
        ...
        if (errorFlag.good())
        {
            /* ... and convert it to a character string (hex mode) */
            char buffer[32];
            //G.Barrand : clang : h -> hh to rm warnings.
            sprintf(buffer, "%2.2hhx", uint8Val);
            /* assign result */
            stringVal = buffer;
        }
    }
...
** CVS/RCS Log:
** $Log: dcvrobow.cc,v $
** Revision 1.1  2013/07/02 07:16:20  barrand
** *** empty log message ***
**
** Revision 1.1.1.1  2013/04/16 19:23:36  barrand
**
**
