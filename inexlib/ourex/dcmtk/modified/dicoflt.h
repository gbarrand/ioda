 ...
 *  Update Date:      $Date: 2013/04/16 19:23:55 $
 ...
 private:
                     ...
                     const int vert)
    {
      if (DiColorPixelTemplate<T>::Init(pixel)) //G.Barrand
        {
            if (horz && vert)
	      DiFlipTemplate<T>::flipHorzVert(pixel, this->Data); //G.Barrand
            else if (horz)
	      DiFlipTemplate<T>::flipHorz(pixel, this->Data); //G.Barrand
            else if (vert)
	      DiFlipTemplate<T>::flipVert(pixel, this->Data); //G.Barrand
        }
    }
};
