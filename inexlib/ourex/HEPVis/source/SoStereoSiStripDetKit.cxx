...
SO_KIT_SOURCE(SoStereoSiStripDetKit) 

// initClass
void SoStereoSiStripDetKit::initClass(){
  SO_KIT_INIT_CLASS(SoStereoSiStripDetKit,SoBaseKit,"BaseKit");
}

// Constructor
SoStereoSiStripDetKit::SoStereoSiStripDetKit() {
  SO_KIT_CONSTRUCTOR(SoStereoSiStripDetKit);
  SO_NODE_ADD_FIELD(alternateRep, (NULL));
  SO_NODE_ADD_FIELD(stereoAngle,   (0));
  ...
  SO_KIT_ADD_CATALOG_ENTRY(phiStripsAppearance, SoAppearanceKit, TRUE, stripsSeparator,\0,  TRUE);
  ...
  SO_KIT_ADD_CATALOG_ENTRY(stretch,          SoScale,         FALSE, stripsSeparator,\0,  FALSE);
  SO_KIT_ADD_CATALOG_ENTRY(rot,              SoRotationXYZ,   FALSE, stripsSeparator,\0,  FALSE);
  SO_KIT_ADD_CATALOG_ENTRY(zStripsAppearance, SoAppearanceKit, TRUE, stripsSeparator,\0,  TRUE);
...
SbBool SoStereoSiStripDetKit::affectsState() const {
  return FALSE;
}

void SoStereoSiStripDetKit::createInitialStripDet() {
  ...
  ScaleCalc->a.connectFrom(&stereoAngle);
  ScaleCalc->expression.set1Value(0,"oA=vec3f(1.0,1.0,1.0/cos(a))");

  // Mr. Rotation
  SoRotationXYZ *MisterRXYZ = (SoRotationXYZ *) rot.getValue();
  MisterRXYZ->axis=SoRotationXYZ::X;
  MisterRXYZ->angle.connectFrom(&stereoAngle);

  // Put Mr. Stretch there
  SoScale *MisterStretch = (SoScale *) stretch.getValue();
  MisterStretch->scaleFactor.connectFrom(&ScaleCalc->oA);
  ...
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
			       SoStereoSiStripDetKit::activateDetector,
			       this);
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
			       SoStereoSiStripDetKit::deactivateDetector  ,
			       this);
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
			       SoStereoSiStripDetKit::translateDetector,
			       this);
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
			       SoStereoSiStripDetKit::detranslateDetector  ,
			       this);
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
			       SoStereoSiStripDetKit::activateStrip,
			       this);
...
void SoStereoSiStripDetKit::translateDetector(void *userData, SoEventCallback *eventCB){

  // Was the event previously handled?
  if (eventCB->isHandled()) return;

  // Which Strip Detector is this being called for?
  SoStereoSiStripDetKit *This = (SoStereoSiStripDetKit *) userData;
  ...
  if (pickedPoint) {
    ...
    for (int i=0;i<path->getLength();i++) {
      ...
      if (ancestorNode->isOfType(SoStereoSiStripDetKit::getClassTypeId()))  break;
    }
...
void SoStereoSiStripDetKit::detranslateDetector(void *userData, SoEventCallback *eventCB){
  // Was the event previously handled?
  if (eventCB->isHandled()) return;

  // Which Strip Detector is this being called for?
  SoStereoSiStripDetKit *This = (SoStereoSiStripDetKit *) userData; // Just as if ...
...
void SoStereoSiStripDetKit::activateDetector(void *userData, SoEventCallback *eventCB){
...
void SoStereoSiStripDetKit::deactivateDetector(void *userData, SoEventCallback *eventCB){
  // Was the event previously handled?
  if (eventCB->isHandled()) return;
...
void SoStereoSiStripDetKit::activateStrip(void *userData, SoEventCallback *eventCB){
...
void SoStereoSiStripDetKit::generateAlternateRep(SoAction* aAction) {
...
// clearAlternateRep
void SoStereoSiStripDetKit::clearAlternateRep() {
  SoSiStrips      *ThePhiStrips                   = (SoSiStrips *)      phiStrips.getValue();
...
void SoStereoSiStripDetKit::setAlignable(SbBool flag) {
 isAlignable=flag;
}

SoTransform *SoStereoSiStripDetKit::getLocalTransform () const {
