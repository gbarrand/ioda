...
SO_KIT_SOURCE(SoSiStripWedgeDetKit) 

// initClass
void SoSiStripWedgeDetKit::initClass(){
  SO_KIT_INIT_CLASS(SoSiStripWedgeDetKit,SoBaseKit,"BaseKit");
}

// Constructor
SoSiStripWedgeDetKit::SoSiStripWedgeDetKit() {
  SO_KIT_CONSTRUCTOR(SoSiStripWedgeDetKit);
...
SbBool SoSiStripWedgeDetKit::affectsState() const {
  return FALSE;
}

void SoSiStripWedgeDetKit::createInitialStripDet() {
  ...
  StripCalc3->expression.set1Value(1, "oA=vec3f(0,-a + (c*(a+b))/d + (a-b)/2 + e,(b*b - a*a)/(2*d))"); // sin(oa)*(a+b)/2)");
  StripCalc3->expression.set1Value(2, "oc=(a+b)");
  ...
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
                               SoSiStripWedgeDetKit::activateDetector,
                               this);
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
                               SoSiStripWedgeDetKit::deactivateDetector  ,
                               this);
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
                               SoSiStripWedgeDetKit::translateDetector,
                               this);
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
                               SoSiStripWedgeDetKit::detranslateDetector  ,
                               this);
  myCallback->addEventCallback(SoMouseButtonEvent::getClassTypeId(),
                               SoSiStripWedgeDetKit::activateStrip,
                               this);
...
void SoSiStripWedgeDetKit::translateDetector(void *userData, SoEventCallback *eventCB){

  // Was the event previously handled?
  if (eventCB->isHandled()) return;

  // Which Strip Detector is this being called for?
  SoSiStripWedgeDetKit *This = (SoSiStripWedgeDetKit *) userData;
  ...
  if (pickedPoint) {
    ...
    for (int i=0;i<path->getLength();i++) {
      ...
      if (ancestorNode->isOfType(SoSiStripWedgeDetKit::getClassTypeId()))  break;
    }
...
void SoSiStripWedgeDetKit::detranslateDetector(void *userData, SoEventCallback *eventCB){
  // Was the event previously handled?
  if (eventCB->isHandled()) return;

  // Which Strip Detector is this being called for?
  SoSiStripWedgeDetKit *This = (SoSiStripWedgeDetKit *) userData; // Just as if ...
...
void SoSiStripWedgeDetKit::activateDetector(void *userData, SoEventCallback *eventCB){
...
void SoSiStripWedgeDetKit::deactivateDetector(void *userData, SoEventCallback *eventCB){
  // Was the event previously handled?
  if (eventCB->isHandled()) return;
...
void SoSiStripWedgeDetKit::activateStrip(void *userData, SoEventCallback *eventCB){
...
void SoSiStripWedgeDetKit::generateAlternateRep(SoAction* aAction) {
...
// clearAlternateRep
void SoSiStripWedgeDetKit::clearAlternateRep() {
  SoSiWedgeStrips      *ThePhiStrips                   = (SoSiWedgeStrips *)      phiStrips.getValue();
...
void SoSiStripWedgeDetKit::setAlignable(SbBool flag) {
 isAlignable=flag;
}

SoTransform *SoSiStripWedgeDetKit::getLocalTransform () const {
