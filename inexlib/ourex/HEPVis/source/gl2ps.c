...
#include "HEPVis_gl2ps.h" /*G.Barrand*/
/*G.Barrand #include "gl2ps.h" */
...
#if defined(HEPVIS_GL2PS_HAVE_ZLIB)
#include <ourex_zlib.h>
/*G.Barrand #include <zlib.h>*/
#endif

#if defined(HEPVIS_GL2PS_HAVE_LIBPNG)
#include <ourex_png.h>
/*G.Barrand #include <png.h>*/
#endif
...
static void gl2psPrintPostScriptPrimitive(void *data)
{
  ...
  case GL2PS_LINE :
    ...
    gl2psPrintPostScriptDash(prim->pattern, prim->factor, (char*)"setdash");
    gl2psPrintPostScriptColor(prim->verts[0].rgba);
...
static void gl2psPrintPostScriptBeginViewport(GLint viewport[4])
{
  ...
  gl2psResetPostScriptColor(); /*G.Barrand*/
  ...
  if(gl2ps->options & GL2PS_DRAW_BACKGROUND){
    ...
    /*G.Barrand : do the below only if DRAW_BACKGROUND */
...
static void gl2psPDFgroupListWriteMainStream(void)
{
  ...
  for(i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i){
    ...
    case GL2PS_LINE:
      ...
      gl2ps->streamlength += gl2psPrintPDFLineWidth(prim->width);
      gl2ps->streamlength += gl2psPrintPDFStrokeColor(prim->verts[0].rgba);
      gl2ps->streamlength += gl2psPrintPostScriptDash(prim->pattern, prim->factor, (char*)"d");
      /* start new path */
      gl2ps->streamlength += 
        gl2psPrintf("%f %f m\n", 
                    prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
...
GL2PSDLL_API GLint gl2psGetOptions(GLint* options) /*G.Barrand*/
{
  if(!gl2ps) {
    *options = 0;
