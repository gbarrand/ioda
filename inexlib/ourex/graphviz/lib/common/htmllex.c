...
{
    return gv_strcasecmp(i->name, j->name);
}
...
static int stylefn(htmltbl_t * p, char *v)
{
    int rv = 0;
    char c = toupper(*v);
    if ((c == 'R') && !gv_strcasecmp(v + 1, "OUNDED"))
	p->style = ROUNDED;
    else {
...
static int fixedsizefn(htmldata_t * p, char *v)
{
    ...
    if ((c == 'T') && !gv_strcasecmp(v + 1, "RUE"))
	p->flags |= FIXED_FLAG;
    else if ((c != 'F') || gv_strcasecmp(v + 1, "ALSE")) {
...
static int valignfn(htmldata_t * p, char *v)
{
    int rv = 0;
    char c = toupper(*v);
    if ((c == 'B') && !gv_strcasecmp(v + 1, "OTTOM"))
	p->flags |= VALIGN_BOTTOM;
    else if ((c == 'T') && !gv_strcasecmp(v + 1, "OP"))
	p->flags |= VALIGN_TOP;
    else if ((c != 'M') || gv_strcasecmp(v + 1, "IDDLE")) {
...
static int halignfn(htmldata_t * p, char *v)
{
    int rv = 0;
    char c = toupper(*v);
    if ((c == 'L') && !gv_strcasecmp(v + 1, "EFT"))
	p->flags |= HALIGN_LEFT;
    else if ((c == 'R') && !gv_strcasecmp(v + 1, "IGHT"))
	p->flags |= HALIGN_RIGHT;
    else if ((c != 'C') || gv_strcasecmp(v + 1, "ENTER")) {
...
static int cell_halignfn(htmldata_t * p, char *v)
{
	...
	p->flags |= HALIGN_RIGHT;
    else if ((c == 'T') && !gv_strcasecmp(v + 1, "EXT"))
	p->flags |= HALIGN_TEXT;
    else if ((c != 'C') || gv_strcasecmp(v + 1, "ENTER"))
	rv = 1;
    if (rv)
...
static int alignfn(int *p, char *v)
{
    int rv = 0;
    char c = toupper(*v);
    if ((c == 'R') && !gv_strcasecmp(v + 1, "IGHT"))
	*p = 'r';
    else if ((c == 'L') || !gv_strcasecmp(v + 1, "EFT"))
	*p = 'l';
    else if ((c == 'C') || gv_strcasecmp(v + 1, "ENTER")) 
	*p = 'n';
    else {
...
{
    if (gv_strcasecmp(name, "TABLE") == 0) {
	htmllval.tbl = mkTbl(atts);
	state.inCell = 0;
	state.tok = T_table;
    } else if ((gv_strcasecmp(name, "TR") == 0)
	       || (gv_strcasecmp(name, "TH") == 0)) {
	state.inCell = 0;
	state.tok = T_row;
    } else if (gv_strcasecmp(name, "TD") == 0) {
	state.inCell = 1;
	htmllval.cell = mkCell(atts);
	state.tok = T_cell;
    } else if (gv_strcasecmp(name, "FONT") == 0) {
	htmllval.font = mkFont(atts, 0, 0);
	state.tok = T_font;
    } else if (gv_strcasecmp(name, "B") == 0) {
	htmllval.font = mkFont(0, HTML_BF, 0);
	state.tok = T_bold;
    } else if (gv_strcasecmp(name, "U") == 0) {
	htmllval.font = mkFont(0, HTML_UL, 1);
	state.tok = T_underline;
    } else if (gv_strcasecmp(name, "I") == 0) {
	htmllval.font = mkFont(0, HTML_IF, 0);
	state.tok = T_italic;
    } else if (gv_strcasecmp(name, "SUP") == 0) {
	htmllval.font = mkFont(0, HTML_SUP, 0);
	state.tok = T_sup;
    } else if (gv_strcasecmp(name, "SUB") == 0) {
	htmllval.font = mkFont(0, HTML_SUB, 0);
	state.tok = T_sub;
    } else if (gv_strcasecmp(name, "BR") == 0) {
	mkBR(atts);
	state.tok = T_br;
    } else if (gv_strcasecmp(name, "IMG") == 0) {
	htmllval.img = mkImg(atts);
	state.tok = T_img;
    } else if (gv_strcasecmp(name, "HTML") == 0) {
	state.tok = T_html;
    } else {
	lexerror(name);
    }
