...
#ifdef _WIN32
/*G.Barrand : #include "libltdl/lt_system.h" */
#define R_OK 4 /*G.Barrand : _WIN32 */
#endif
#ifndef _WIN32
#include <unistd.h>
#endif
...
boolean mapBool(char *p, boolean dflt)
{
    if (!p || (*p == '\0'))
	return dflt;
    if (!gv_strcasecmp(p, "false"))
	return FALSE;
    if (!gv_strcasecmp(p, "no"))
	return FALSE;
    if (!gv_strcasecmp(p, "true"))
	return TRUE;
    if (!gv_strcasecmp(p, "yes"))
	return TRUE;
    if (isdigit(*p))
	return atoi(p);
    else
...
{
    return ((g == g->root) || (!gv_strncasecmp(agnameof(g), "cluster", 7)));
}
...
int edgeType (char* s, int dflt)
{
    ...
    case 'c' :
    case 'C' :
	if (!gv_strcasecmp (s+1, "ompound"))
	    et = ET_COMPOUND;
	break;
    case 'f' :
    case 'F' :
	if (!gv_strcasecmp (s+1, "alse"))
	    et = ET_LINE;
	break;
    case 'l' :
    case 'L' :
	if (!gv_strcasecmp (s+1, "ine"))
	    et = ET_LINE;
	break;
    case 'n' :
    case 'N' :
	if (!gv_strcasecmp (s+1, "one")) return et;
	if (!gv_strcasecmp (s+1, "o")) return ET_LINE;
	break;
    case 'o' :
    case 'O' :
	if (!gv_strcasecmp (s+1, "rtho"))
	    et = ET_ORTHO;
	break;
    case 'p' :
    case 'P' :
	if (!gv_strcasecmp (s+1, "olyline"))
	    et = ET_PLINE;
	break;
    case 's' :
    case 'S' :
	if (!gv_strcasecmp (s+1, "pline"))
	    et = ET_SPLINE;
	break;
    case 't' :
    case 'T' :
	if (!gv_strcasecmp (s+1, "rue"))
	    et = ET_SPLINE;
	break;
    case 'y' :
    case 'Y' :
	if (!gv_strcasecmp (s+1, "es"))
	    et = ET_SPLINE;
	break;
    }
    if (!et) {
...
int gv_strcasecmp(const char *s1, const char *s2)
{
    while ((*s1 != '\0')
...
int gv_strncasecmp(const char *s1, const char *s2, unsigned int n)
{
    if (n == 0)
	return 0;
