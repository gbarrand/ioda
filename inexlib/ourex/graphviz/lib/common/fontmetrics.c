...
estimate_textlayout(graph_t *g, textpara_t * para, char **fontpath)
{
    ...
    if (!gv_strncasecmp(para->fontname, "cour", 4)) {
	fp = "[internal courier]";
	Fontwidth = courFontWidth;
    } else if (!gv_strncasecmp(para->fontname, "arial", 5)
	       || !gv_strncasecmp(para->fontname, "helvetica", 9)) {
	fp = "[internal arial]";
...
{
    return (gv_strcasecmp(((PostscriptAlias*)a)->name, ((PostscriptAlias*)b)->name));
}
...
static PostscriptAlias* translate_postscript_fontname(char* fontname)
{
    ...
    if (key.name == NULL || gv_strcasecmp(key.name, fontname)) {
        key.name = fontname;
