...
#include "gv_config.h" /*G.Barrand*/

static char* colorscheme;

/*G.Barrand : #ifdef WIN32*/
#ifndef HAVE_STRCASECMP
extern int gv_strcasecmp(const char *s1, const char *s2);
#endif
#ifndef HAVE_STRNCASECMP
extern int gv_strncasecmp(const char *s1, const char *s2, unsigned int n);
#endif
/*#endif*/
...
{
    return gv_strcasecmp(((hsvrgbacolor_t *) p0)->name, ((hsvrgbacolor_t *) p1)->name);
}
...
#define ISNONDFLT(s) ((s) && *(s) && gv_strncasecmp(DFLT_SCHEME, s, DFLT_SCHEME_LEN-1))
...
static char* resolveColor (char* str)
{
	...
	c2 = str+1;
	    ...
	    else if (gv_strncasecmp(DFLT_SCHEME, c2, DFLT_SCHEME_LEN)) s = str;
	    else s = ss + 1;
	}
	else s = c2;
    }
