...
static char* dotneato_basename (char* path)
{
    ...
    {
	char* dotp = strrchr (s, '.');
	if (dotp && !gv_strcasecmp(dotp+1,"exe")) *dotp = '\0';
    }
#endif
    while (*s) s++; s--;
...
static int findCharset (graph_t * g)
{
    ...
    if (!gv_strcasecmp(p,"latin-1")
	|| !gv_strcasecmp(p,"latin1")
	|| !gv_strcasecmp(p,"l1")
	|| !gv_strcasecmp(p,"ISO-8859-1")
	|| !gv_strcasecmp(p,"ISO_8859-1")
	|| !gv_strcasecmp(p,"ISO8859-1")
	|| !gv_strcasecmp(p,"ISO-IR-100"))
		enc = CHAR_LATIN1; 
    else if (!gv_strcasecmp(p,"big-5")
	|| !gv_strcasecmp(p,"big5")) 
		enc = CHAR_BIG5; 
    else if (!gv_strcasecmp(p,"utf-8")
	|| !gv_strcasecmp(p,"utf8"))
		enc = CHAR_UTF8; 
    else {
