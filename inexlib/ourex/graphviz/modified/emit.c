...
{
    static char* save_locale = NULL; /*G.Barrand : = NULL */
    static int cnt = 0; /*G.Barrand : = 0 */

    if (set) {
	cnt++;
	if (cnt == 1) {
          /*G.Barrand : Android : setlocale returns NULL */
          /*save_locale = strdup (setlocale (LC_NUMERIC, NULL));*/
          char* s = setlocale (LC_NUMERIC, NULL);
          if(s!=NULL) save_locale = strdup (s);
...
{
    static GVJ_t *prevjob = NULL; /*G.Barrand : = NULL */
    GVJ_t *job, *firstjob;
