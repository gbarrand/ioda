...
#undef HAVE_AGL //G.Barrand : else, we tie to QuickDraw which is deprecated.
...
void aglglue_context_bind_pbuffer(void * ctx) { //G.Barrand
  assert(FALSE); 
}

void aglglue_context_release_pbuffer(void * ctx) { //G.Barrand
  assert(FALSE); 
}

SbBool aglglue_context_pbuffer_is_bound(void * ctx) { //G.Barrand
  assert(FALSE); 
  return FALSE;
}

SbBool aglglue_context_can_render_to_texture(void * ctx) { //G.Barrand
  assert(FALSE); 
  return FALSE;
}

void aglglue_cleanup(void) //G.Barrand
{
  assert(FALSE); 
}
