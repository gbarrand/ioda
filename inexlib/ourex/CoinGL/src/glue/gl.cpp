...
#ifdef HAVE_CGL //G.Barrand
#include <OpenGL/OpenGL.h>
#endif

#ifdef HAVE_GLX
#ifndef HAVE_DYNAMIC_LINKING      /*G.Barrand*/
#define GLX_GLXEXT_PROTOTYPES 1  
#define GL_GLEXT_PROTOTYPES 1     
/* ubuntu (on VirtualBox) : some glext functions are not here : */
...
#include <Inventor/C/glue/gl_cgl.h> //G.Barrand
...
//G.Barrand : begin :
// on some Linuxes (SLC6, CentOS), cpp GL_EXT_polygon_offset, GL_EXT_subtexture are defined but 
// the functions glPolygonOffsetEXT, glTexSubImage2DEXT are not in the GL lib. Then we just bypass these (on all platforms).
#ifdef __linux__
...
glglue_cleanup(void)
{
  ...
  wglglue_cleanup();
#elif defined(HAVE_CGL) //G.Barrand
  cglglue_cleanup();
#endif
}
...
cc_glglue_instance(int contextid)
{
  ...
  if (!found) {
    ...
    */
    ...
    /*
      G.Barrand : Darwin : XQuartz 2.3.2 : add this line
      G.Barrand : else it is set to GL_INVALID_OPERATION
      G.Barrand : and the below test will not pass.
      glGetError(); 
    */
...
#elif defined(HAVE_AGL) || defined(HAVE_CGL) //G.Barrand
