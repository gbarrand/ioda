...
/*G.Barrand : #include <Inventor/system/gl.h>*/
#include <Inventor/system/glu-headers.h> /*G.Barrand*/
  ...
  */
  {
    ...
    if (!versionstr) {
      versionstr = (const char *) gi->gluGetString(GLU_VERSION);
    }
    GLUWrapper_set_version((const GLubyte *) versionstr);
  }
