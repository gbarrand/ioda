                            ...
                            const int * indices)
{
  ...
  int childidx = 0;
  SoNode * node;
  for (int i = 0; (i < numindices) && !action->hasTerminated(); i++) { //G.Barrand : have () in i<.
    int stop = indices[i];
    for (; (childidx < stop) && !action->hasTerminated(); childidx++) { //G.Barrand : have () in childidx<.
      // we are off path. Check if node affects state before traversing
      if(childidx<getLength()) { //G.Barrand.
        node = (*this)[childidx];
        ...
        SoDebugError::postInfo("SoChildList::traverseInPath",
                               "(%p) childidx %d exceeds list length %d",this, childidx,getLength());
      } //G.Barrand
    }
    ...
    if (!action->hasTerminated()) {
      ...
      if(childidx<getLength()) { //G.Barrand.
        ...
        SoDebugError::postInfo("SoChildList::traverseInPath(2)",
                               "(%p) childidx %d exceeds list length %d",this, childidx,getLength());
      } //G.Barrand
      childidx++;
    }
