...
SoGlyphP::setup3DFontData(void)
{
  ...
  if (character <= 32 || character >= 127) {
    ...
    if (vector_glyph == NULL) {
      ...
      PUBLIC(this)->setCoords((SbVec2f *)coin_default3dfont_get_coords()[idx]);
      PUBLIC(this)->setFaceIndices((int*) coin_default3dfont_get_faceidx()[idx]);
      PUBLIC(this)->setEdgeIndices((int*) coin_default3dfont_get_edgeidx()[idx]);
    } 
    else {
      // Install truetype font

      PUBLIC(this)->setCoords((SbVec2f *)cc_flw_get_vector_glyph_coords(vector_glyph));
      PUBLIC(this)->setFaceIndices((int*) cc_flw_get_vector_glyph_faceidx(vector_glyph));
      PUBLIC(this)->setEdgeIndices((int*) cc_flw_get_vector_glyph_edgeidx(vector_glyph));
    }
  }
}
