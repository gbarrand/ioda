...
// Coin-2: necessary for the SoTexture2::enableCompressedTexture hack.
                      ...
                      const SoField * field)
{
  ...
  if (!this->hasField(name)) {
    ...
    // Coin-2 HACK, since SoTexture2::enableCompressedTexture is not
    // part of SoTexture2 (but rather SoTexture2P).
    const SbBool isSoTexture2enableCompressedTexture = 
      (base->isOfType(SoTexture2::getClassTypeId())) &&
      (name == SbName("enableCompressedTexture"));
    if (isSoTexture2enableCompressedTexture) {
      vbase = (char *)(((SoTexture2 *)base)->pimpl);
    }
    // HACK end

    const char * vfield = (char *)field;
...
SoFieldData::getField(const SoFieldContainer * object, int index) const
{
  ...
  // part of SoTexture2 (but rather SoTexture2P).
  const SbBool isSoTexture2enableCompressedTexture = 
    (object->isOfType(SoTexture2::getClassTypeId())) &&
    (this->fields[index]->name == SbName("enableCompressedTexture"));
  if (isSoTexture2enableCompressedTexture) {
    fieldptr = (char *)(((SoTexture2 *)object)->pimpl);
  }
  // HACK end
...
SoFieldData::getIndex(const SoFieldContainer * fc, const SoField * field) const
{
  ...
  ptrdiff_t ptroffset = vfield - vbase;
  int i;

  for (i=0; i < this->fields.getLength(); i++) {
    if (this->fields[i]->ptroffset == ptroffset) { return i; }
  }

  // Coin-2 HACK, since SoTexture2::enableCompressedTexture is not
  // part of SoTexture2 (but rather SoTexture2P).
  const SbBool maybeSoTexture2enableCompressedTexture = 
    (fc->isOfType(SoTexture2::getClassTypeId())) &&
    (field->getTypeId().getName() == "SoSFBool");

  if (maybeSoTexture2enableCompressedTexture) {
    vbase = (char *)(((SoTexture2 *)fc)->pimpl);
    ptroffset = vfield - vbase;
    for (i=0; i < this->fields.getLength(); i++) {
      if ((this->fields[i]->ptroffset == ptroffset) &&
          (this->fields[i]->name == "enableCompressedTexture")) {
        return i;
      }
    }
  }
