...
public:
  ...
  // Needed since popCurPath() is inline in Coin-2. This workaround is
  // not needed in Coin development.
  void fixMissingCompactPathListPops(const SoTempPath & currentpath) {
    int len = currentpath.getLength();
    while (this->applieddata.pathlistdata.compactlist->getDepth() > len) {
...
{
  // Detect missing popCurPath(PathCode) calls. See comment in
  // fixMissingCompactPathListPops()
  if (this->currentpathcode == IN_PATH &&
      this->getWhatAppliedTo() != PATH &&
      THIS->applieddata.pathlistdata.compactlist != NULL) {
    THIS->fixMissingCompactPathListPops(this->currentpath);
  }
