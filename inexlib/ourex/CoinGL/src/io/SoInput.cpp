...
typedef SbHash<SoInputP *, const SoInput *> Public2PrivateMap;
static Public2PrivateMap * soinput_private_data_dict = NULL;
...
soinput_get_private_data(const SoInput * thisp)
{
  if (soinput_private_data_dict == NULL) {
    soinput_private_data_dict = new Public2PrivateMap;
    coin_atexit(soinput_private_data_cleanup, 0);
  }
  SoInputP * pimpl;
  if (!soinput_private_data_dict->get(thisp, pimpl)) {
    pimpl = new SoInputP((SoInput*) thisp);
    (void) soinput_private_data_dict->put(thisp, pimpl);
  }
  return pimpl;
}

static void
soinput_delete_private_data(const SoInput * thisp)
{
  if (soinput_private_data_dict) {
    SoInputP * pimpl;
    if (soinput_private_data_dict->get(thisp, pimpl)) {
      soinput_private_data_dict->remove(thisp);
      delete pimpl;
    }
  }
}

#define PRIVATE(obj) (soinput_get_private_data(obj))
...
SoInput_FileInfo * 
SoInputP::getTopOfStackPopOnEOF(SoInput * owner)
{
...
SoInput::~SoInput(void)
{
  ...
  // delete and remove private data from private data dict
  soinput_delete_private_data(this);
}
...
SoInput::get(char & c)
{
  ...
  SoInput_FileInfo * fi = SoInputP::getTopOfStackPopOnEOF(this);
...
/*!
  ...
  This function is not virtual in Coin 2.* for ABI reasons.
*/
...
SoInput::checkHeader(SbBool bValidateBufferHeader)
{
  ...
  if (this->filestack.getLength() == 1 && 
      ...
      !(pimpl = PRIVATE(this))->usingstdin) {

    pimpl->usingstdin = TRUE;
  }
