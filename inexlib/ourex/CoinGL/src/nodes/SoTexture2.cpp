...
/*!
  ...
  Note that in Coin-2 this is a hidden field and can only be accessed
  through SoFieldContainer::getField().
...
class SoTexture2P {
public:
  // This field part of the private implementation class, since we
  // cannot change the ABI. In Coin 3, it is part of the public
  // interface.
  //
  // (The field is present in Coin 2 because it was useful for the SIM
  // Vispo project.)
  //
  // Note: let this be at memory offset 0, so we make sure it doesn't
  // crash with any of the offsets for the fields in the SoTexture2
  // class (which inherits a non-null size from its superclasses,
  // which means the first field will be guaranteed to not be at
  // offset 0). This is important because otherwise the hack to
  // accommodate this field in SoFieldData will fail.
...
{
  // Note: due to the hack in SoFieldData to handle the "pimpl"-owned
  // enableCompressedTexture field, this must be done before that
  // field is added.
  ...
  // Replacement for SO_NODE_ADD_FIELD(enableCompressedTexture, (FALSE));
  // since this would break ABI. The field will be public from Coin-3.
  PRIVATE(this)->enableCompressedTexture.setValue(FALSE);
  PRIVATE(this)->enableCompressedTexture.setContainer(this);
  this->fieldData->addField(this, "enableCompressedTexture", 
                            &PRIVATE(this)->enableCompressedTexture);
...
SoTexture2::GLRender(SoGLRenderAction * action)
{
  ...
  if (!PRIVATE(this)->glimagevalid || 
    ...
    if (PRIVATE(this)->enableCompressedTexture.getValue()) {
