...
SoSceneTexture2::SoSceneTexture2(void)
{
  ...
  PRIVATE(this)->type = RGBA8;
  PRIVATE(this)->sceneTransparencyType = NULL;
...
#if 0 // not added in Coin-2. Will break the ABI
...
#endif // not added in Coin-2
}

SoSceneTexture2::~SoSceneTexture2(void)
{
  if (PRIVATE(this)->sceneTransparencyType) PRIVATE(this)->sceneTransparencyType->unref();
  delete PRIVATE(this);
}

/*!  
  Temporary function in Coin 2.5. Used to set the buffer type. In
  Coin-3 this function will be obsoleted and replaced with a field.
*/
void 
SoSceneTexture2::setType(const Type type)
{
  PRIVATE(this)->type = type;
}

/*!  
  Temporary function in Coin 2.5. Used to set the scene transparency typex. In
  Coin-3 this function will be obsoleted and replaced with a field.
*/
void 
SoSceneTexture2::setSceneTransparencyType(SoNode * node)
{
  if (PRIVATE(this)->sceneTransparencyType) {
    PRIVATE(this)->sceneTransparencyType->unref();
  }
  PRIVATE(this)->sceneTransparencyType = node;
  if (node) node->ref();
}
...
SoSceneTexture2P::updateFrameBuffer(SoState * state, const float quality)
{
  ...
  if ((this->fbo_size != size) || (mipmap != this->fbo_mipmap)) {
    ...
    SoSceneTexture2::Type type = (SoSceneTexture2::Type) this->type;
    while (!finished) {
    ...
    if (this->type == SoSceneTexture2::DEPTH) {
