...
/* Modified by Eric.Aubourg@cea.fr for SOPHYA */
...
transform( MD5_CONTEXT *ctx, uint_1 *data )
{
    uint_4 correct_words[16];
    uint_4 A = ctx->A;
    uint_4 B = ctx->B;
    uint_4 C = ctx->C;
    uint_4 D = ctx->D;
    uint_4 *cwp = correct_words;

#ifdef BIG_ENDIAN_HOST
    { int i;
      uint_1 *p2, *p1;
      for(i=0, p1=data, p2=(uint_1*)correct_words; i < 16; i++, p2 += 4 ) {
	p2[3] = *p1++;
	p2[2] = *p1++;
	p2[1] = *p1++;
	p2[0] = *p1++;
...
#endif
#else /* little endian */
    /*#define X(a) do { *(uint_4*)p = hd->##a ; p += 4; } while(0)*/
...
md5_get_info( int algo, size_t *contextsize,
	       uint_1 **r_asnoid, int *r_asnlen, int *r_mdlen,
	       void (**r_init)( void *c ),
	       void (**r_write)( void *c, uint_1 *buf, size_t nbytes ),
	       void (**r_final)( void *c ),
	       uint_1 *(**r_read)( void *c )
	     )
{
    static uint_1 asn[18] = /* Object ID is 1.2.840.113549.2.5 */
    ...
    *r_write = (void (*)(void *, uint_1*, size_t))md5_write;
    *r_final = (void (*)(void *))md5_final;
    *r_read  = (uint_1 *(*)(void *))md5_read;
...
static
#endif
const char * const gnupgext_version = "MD5 ($Revision: 1.6 $)";
