...
inline POutPersist& operator << (POutPersist& os, String16 const & fms)
{ os.PutBytes(fms.cbuff_ptr(),16);  return(os); }
inline PInPersist& operator >> (PInPersist& is, String16 & fms)
{ size_t N = 16;char buff[16];  is.GetBytes(buff,N); buff[N-1]='\0'; fms=buff; return(is); }

inline POutPersist& operator << (POutPersist& os, String64 const & fms)
{ os.PutBytes(fms.cbuff_ptr(),64);  return(os); }
inline PInPersist& operator >> (PInPersist& is, String64 & fms)
{ size_t N = 64;char buff[64];  is.GetBytes(buff,N); buff[N-1]='\0'; fms=buff; return(is); }

  /*
inline POutPersist& operator << (POutPersist& os, FlagVector16 const & fgv)
{ os.PutBytes(fgv.bit_array(),16);  return(os); }
inline PInPersist& operator >> (PInPersist& is, FlagVector16 & fgv)
{ is.GetBytes(fgv.bit_array(),16); return(is); }

inline POutPersist& operator << (POutPersist& os, FlagVector64 const & fgv)
{ os.PutBytes(fgv.bit_array(),64);  return(os); }
inline PInPersist& operator >> (PInPersist& is, FlagVector64 & fgv)
{ is.GetBytes(fgv.bit_array(),64); return(is); }
  */
#endif
